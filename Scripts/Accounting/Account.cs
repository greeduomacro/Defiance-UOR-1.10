...
namespace Server.Accounting
{
	public class Account : IAccount, IComparable, IComparable<Account>
	{
		...
		private List<IPAddress> m_LoginIPs;
		private HardwareInfo m_HardwareInfo;
		private FeatureFlags m_SFO;
...
//		private string m_SecurityAnswer;
		...
		/// <summary>
		/// Current e-mail address of the account.
		/// </summary>
		public string Email{ get { return m_Email; } }

		/// <summary>
		/// Current local language of the account.
		/// </summary>
		public string Language{ get { return m_Language; } set{ m_Language = value; } }

		/// <summary>
		/// Current local timezone of the account.
		/// </summary>
		public int TimeZoneIndex{ get { return m_TimeZone; } set{ m_TimeZone = value; } }

#if ( Framework_3_5 || Framework_4_0 )
		public TimeZoneInfo TimeZone{ get{ return Utility.GetTimeZone( m_TimeZone ); } }
#endif

		/// <summary>
		/// Account's security question for password recovery.
		/// </summary>
		public string SecurityQuestion{ get { return m_SecurityQuestion; } set{ m_SecurityQuestion = value; } }

		/// <summary>
		/// Account's security answer for password recovery.
		/// </summary>
//		public string SecurityAnswer{ get { return m_SecurityAnswer; } set{ m_SecurityAnswer = value; } }
		...
		/// </summary>
		public List<IPAddress> LoginIPs
		{
		...
		/// List of IP addresses which have successfully logged into this account.
		/// </summary>
		public IPAddress[] GetLoginIPs()
		{
			return m_LoginIPs.ToArray();
		}
		...
		{
			if ( Server.Misc.Email.IsValid( address ) )
				m_Email = address;
		}
		...
		public TimeSpan TotalAcctGameTime
		{
		...
		{
			//If null is specified for from, then it clears the ban tags, otherwise it sets them to infinite
			SetBanTags( from, DateTime.MinValue, from == null ? TimeSpan.Zero : TimeSpan.MaxValue );
		}
...
//			Confirmation.VerifyEmail( m, acc );
		...
		public static List<IPAddress> LoadAddressList( XmlElement node )
		{
			List<IPAddress> list;
...
//					{
						...
						if( IPAddress.TryParse( Utility.GetText( ip, null ), out address ) )
							list.Add( Utility.Intern( address ) );
//					}
				}
/*
				if ( count != list.Length )
...
*/
		...
		public void Save( XmlTextWriter xml )
		{
			...
			if ( m_SFO != FeatureFlags.None )
			{
				xml.WriteStartElement( "SFO" );
				xml.WriteString( XmlConvert.ToString( (int)m_SFO ) );
				xml.WriteEndElement();
			}
			...
			if ( m_LoginIPs.Count > 0 )
			{
				...
				xml.WriteAttributeString( "count", m_LoginIPs.Count.ToString() );

				for ( int i = 0; i < m_LoginIPs.Count; ++i )
				{
					if ( m_LoginIPs[i] != null )
					{
