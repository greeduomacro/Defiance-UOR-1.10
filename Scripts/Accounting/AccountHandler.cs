			...
			try
			{
				...
				else
				{
					...
					else if ( PageQueue.CheckAllowedToPage( from ) )
					{
						...
						PageQueue.Enqueue( new PageEntry( from, String.Format( "[Automated: Change Password]<br>Desired password: {0}<br>Current IP address: {1}<br>Account IP address: {2}", pass, ipAddress, acct.LoginIPs[0] ), PageType.Account ) );
					}
