...
using Server.Misc;
//using Server.CounterStrike.Misc;
...
namespace Server.Commands
{
	public class CommandHandlers
	{
		public static void Initialize()
		{
            ...
            //Al: PageSpy
            Register("PageSpy", AccessLevel.Seer, new CommandEventHandler(PageSpy_OnCommand));
			...
			Register( "CBCast", AccessLevel.GameMaster, new CommandEventHandler( CBroadcastMessage_OnCommand ) );
			Register( "CBC", AccessLevel.GameMaster, new CommandEventHandler( CBroadcastMessage_OnCommand ) );
			Register( "CB", AccessLevel.GameMaster, new CommandEventHandler( CBroadcastMessage_OnCommand ) );
			...
			Register( "Stats", AccessLevel.Administrator, new CommandEventHandler( Stats_OnCommand ) );
		...
		private static void SpeedBoost_OnCommand( CommandEventArgs e )
		{
			...
			if ( e.Length <= 1 )
			{
				bool on = e.GetBoolean( 0 ) || e.ArgString.Trim().ToLower() == "on";
				if ( e.Length == 1 && !on )
				{
...
*/
		...
		{
			e.Mobile.SendMessage( "Choose an object to move." );
		...
		{
			e.Mobile.SendMessage( "World save has been started." );

			Misc.AutoSave.ThreadedBackup();
			Misc.AutoSave.Save();
		}
...
*/
		...
		[Usage( "CBCast <text>" )]
		[Aliases( "CB", "CBC" )]
		[Description( "Broadcasts a message to everyone online without displaying your name." )]
		public static void CBroadcastMessage_OnCommand( CommandEventArgs e )
		{
			BroadcastMessage( AccessLevel.Player, e.Mobile.SpeechHue, e.ArgString );
		}
        ...
        //Al: Pagespy
        //[Usage("PageSpy")]
        //[Description("Toggles your page spying status.")]
        public static void PageSpy_OnCommand(CommandEventArgs e)
        {
            Mobile m = e.Mobile;
            if (m is PlayerMobile)
            {
                PlayerMobile pm = (PlayerMobile)m;
                pm.PageSpy = !pm.PageSpy;
                m.SendMessage("Your page spy has been turned {0}.", pm.PageSpy ? "on" : "off");
            }
        }
		...
		private class StuckMenuTarget : Target
		{
			...
			protected override void OnTarget( Mobile from, object targeted )
			{
                if (targeted is PlayerMobile) //includes targeted is Mobile
                {
                   /*if (CTFGame.FindTeamFor((PlayerMobile)targeted) != null)
                    {
                        from.SendMessage("You cannot stuck this player because he is in a CTF game. Please use [ctfkick");
                    }
                    else if (CounterStrikeSystem.GetMobilesCSTeam((Mobile)targeted) != CSTeamHandle.None)
                    {
                        from.SendMessage("You cannot stuck this player because he is in a CS game.");
                    }
                    else*/ if (((Mobile)targeted).AccessLevel >= from.AccessLevel && ((Mobile)targeted) != from)
                    {
                        from.SendMessage("That is not accessible.");
                    }
                    else
