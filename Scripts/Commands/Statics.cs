...
namespace Server
{
	public class Statics
	{
		...
		public static void Freeze( Mobile from, Map targetMap, Point3D start3d, Point3D end3d )
		{
			...
			if ( start3d == NullP3D && end3d == NullP3D )
			{
				...
				foreach ( Item item in World.Items.Values )
				{
					...
					if ( item is Static || item is BaseFloor || item is BaseWall )
					{
						...
						Dictionary<Point2D, DeltaState> table;
						mapTable.TryGetValue( itemMap, out table );
			...
			foreach ( KeyValuePair<Map, Dictionary<Point2D, DeltaState>> kvp in mapTable )
			{
				Map map = kvp.Key;
				Dictionary<Point2D, DeltaState> table = kvp.Value;
