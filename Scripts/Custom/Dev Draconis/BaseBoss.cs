...
namespace Server.Mobiles
{
	...
	public class BaseBoss : BaseCreature
	{
		...
		public virtual bool DoSpawnWyvern { get { return false; } } //not on dfi
		...
		public virtual int CanMagicHeal { get { return 0; } }  //not on dfi
		public virtual bool DoTeleHide { get { return false; } } //not on dfi
		public virtual int DoReturnHome { get { return 0; } }  //not on dfi
		...
		{
			if ( from is BaseCreature && DoImmuneToPets)
				amount = (int)(0);

			if ( from is PlayerMobile && DoImmuneToPlayers)
				amount = (int)(0);
		...
		{
			if (caster is BaseCreature && DoWeaponsDoMoreDamage < 1)
			{
		...
		public override void CheckReflect(Mobile caster, ref bool reflect)
		{
			...
			if ( CanCheckReflect == 4 )
				reflect = true;

			if ( 0.25 >= Utility.RandomDouble() && CanCheckReflect == 1 )
				reflect = true;

			if ( 0.50 >= Utility.RandomDouble() && CanCheckReflect == 2 )
				reflect = true;

			if ( 0.75 >= Utility.RandomDouble() && CanCheckReflect == 3 )
				reflect = true;

			if ( caster.Body.IsMale && CanCheckReflect == 5)
				reflect = true;

			if ( caster.Body.IsFemale && CanCheckReflect == 6)
				reflect = true;
			else
		...
		public override void OnThink()
		{
			...
			if ( CanMagicHeal > 0 && DateTime.Now >= m_NextHealTime && this.Hits < this.HitsMax )
			{
				m_NextHealTime = DateTime.Now + TimeSpan.FromSeconds( Utility.RandomMinMax( 30, 45 ) );

				if ( this.Poisoned == false )
				{
					this.Say( "In Vas Mani" );
					this.FixedParticles( 0x376A, 9, 32, 5030, EffectLayer.Waist );
					this.PlaySound( 0x202 );
					this.Hits += CanMagicHeal;
				}
				else
				{
					this.Say( "An Nox" );
			...
			if ( DoTeleHide && this.Hits < this.HitsMax * 0.2 && m_CanHide )
			{
				m_CanHide = false;
				...
				if (map != null)
				{
					...
					for (int i = 0; i < 1; ++i)
					{
						int x = this.X + ( Utility.RandomList( -10,-9, -8, 8, 9, 10 ) );
						int y = this.Y + ( Utility.RandomList( -10,-9, -8, 8, 9, 10 ) );
						int z = this.Z;
						...
						Timer.DelayCall( TimeSpan.FromSeconds( 10.0 ), new TimerCallback( Hide ) );
						Timer.DelayCall( TimeSpan.FromSeconds( 2.0 ), new TimerCallback( CheckRevealed ) );
			...
			if ( DoReturnHome > 0 )
			{
				...
				if ( this.Home != p )
				{
					if ( !this.InRange( this.Home, DoReturnHome ) )
					{
						this.Blessed = true;
						this.CantWalk = true;
						this.FixedParticles( 0x3709, 10, 30, 5052, EffectLayer.LeftFoot );
						this.PlaySound( 0x208 );
						Timer.DelayCall( TimeSpan.FromSeconds( 2.0 ), new TimerCallback( ReturnHome ) );
					}
				}
			}
		...
		public void CheckRevealed()
		{
			...
			else if ( this.Hits > this.HitsMax * 0.75 )
			{
				this.CantWalk = false;
				this.FightMode = FightMode.Closest;
			}
