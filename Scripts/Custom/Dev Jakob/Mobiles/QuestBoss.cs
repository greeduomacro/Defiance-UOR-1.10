...
namespace Server.Mobiles
{
	...
	public class QuestBoss : BaseCreature
	{
		...
		public override void OnGaveMeleeAttack( Mobile defender )
		{
			...
			if ( defender is BaseCreature && m_ProvoPets && 0.33 >= Utility.RandomDouble() )
			{
			...
			if (m_DisarmPlayer && defender is PlayerMobile && 0.25 >= Utility.RandomDouble() )
			{
				Item toDisarm=null;
		...
		public static string[] m_FlowerText = new string[]
			{
				"What a beautiful day to grow flowers.",
				"Here little human, have some flowers to brighten your day.",
				"Rise my beauties and brighten my day.",
				"Awww aren't they pretty?",
				"Flowers for me and flowers for you."
			};
		...
		public override void OnThink()
		{
			...
			if ( DateTime.Now >= m_NextFlowerTime )
			{
				Mobile combatant = this.Combatant;
				...
				if ( combatant != null && combatant.Map == this.Map && combatant.InRange( this, 15 ) )
				{
					...
					Say( m_FlowerText[Utility.Random( m_FlowerText.Length )] );
		...
		public void Token()
		{
			...
			for ( int i = 0; i < 10; ++i )
			{
				...
				if ( Utility.Random( 10 ) < 3 )
				{
					...
					m.SendMessage( "You receive a silver token!" );
				}
				if ( Utility.Random( 10 ) < 7 )
				{
					m.AddToBackpack( new BronzePrizeToken() );
					m.SendMessage( "You receive a bronze token!" );
				}
			}
		}
