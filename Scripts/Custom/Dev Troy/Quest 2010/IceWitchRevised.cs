...
namespace Server.Mobiles
{
	...
	public class IceWitch2 : BaseCreature
	{
		[Constructable]
		public IceWitch2 () : base( AIType.AI_Mage, FightMode.Closest, 10, 1, 0.2, 0.4 )
		{
			...
			PackItem( new Gold( 3000 ) );


			if ( Utility.Random( 1 ) < 25 ) PackItem( new IceCube() );

			AddItem( new GnarledStaff() );
			AddItem( new Server.Items.Skirt( Utility.RandomRedHue() ) );
			AddItem( new Server.Items.Doublet( Utility.RandomRedHue() ) );
			AddItem( new Server.Items.ThighBoots( Utility.RandomRedHue() ) );
		...
		public IceWitch2( Serial serial ) : base( serial )
		{
		}
