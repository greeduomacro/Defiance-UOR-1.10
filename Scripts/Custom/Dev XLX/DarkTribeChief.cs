...
{
	[CorpseName( "a tribe chiefs corpse" )]
	public class DarkTribeChief : BaseCreature
	{
		[Constructable]
		public DarkTribeChief() : base( AIType.AI_Mage, FightMode.Closest, 10, 1, 0.2, 0.4 )
		{
			Name = "DarkTribe Chief";
                        Hue = 1175;
			...
			SetSkill( SkillName.EvalInt, 180.0, 180.0 );
			...
			SetSkill( SkillName.MagicResist, 150.0, 150.0 );
			...
			PackItem( new Bandage( Utility.RandomMinMax( 5, 30 ) ) );
			...
			AddItem( new DeerMask() );
                     ...
                     switch ( Utility.Random( 2 ))
        	             {
           			case 0: PackItem( new DarkTribalPaint() ); break;
		...
		public void BeginSavageDance()
		{
			...
			if ( list.Count >= 3 )
			{
				...
				{
					DarkTribeChief dancer = (DarkTribeChief)list[i];
			...
			if ( list.Count > 0 )
			{
				switch ( Utility.Random( 3 ) )
				{
					case 0: /* greater heal */
					{
						...
						{
							bool isFriendly = ( m is Savage || m is SavageRider || m is DarkTribeChief || m is SavageRidgeback );
		...
		public DarkTribeChief( Serial serial ) : base( serial )
		{
		}
