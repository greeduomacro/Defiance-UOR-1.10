...
namespace Server.Items
{
	...
	{
		// *** Added By Eclipse ***
		private bool m_AllowCombat;
		[CommandProperty(AccessLevel.GameMaster)]
		public bool AllowCombat { get { return m_AllowCombat; } set { m_AllowCombat = value; } }
		// *** ***
			...
			set
			{
				...
				if (value && m_RuneBooks != null && m_RuneBooks.Count > 0)
				{
					...
					if (m_RuneBooks != null && m_RuneBooks.Count > 0)
					{
						...
						Hue = runebook.Hue == 1121 ? 1158 : runebook.Hue;
					}
					else
						Hue = 1158;
					...
					this.PublicOverheadMessage(0, this.Hue, false, "MoongateLibrary Active!");
				}
				else
				{
					m_Active = false;
				...
				else
				{
					if (validateRunebook(from, (Runebook)o))
					{
						...
						if (m_RuneBooks.Count == 1 && m_MoongateLibrary.Active)
							m_MoongateLibrary.Hue = orunebook.Hue == 1121 ? 1158 : orunebook.Hue;
		...
		public bool UseGate(Mobile m)
		{
			...
			// *** Changed By Eclipse ***
			else if ( !m_AllowCombat && ( Server.Spells.SpellHelper.CheckCombat( m )/* || PublicMoongate.CheckCombat( m )*/ ) )
			{
	...
	public class MoongateLibraryGump : Gump
	{
			...
			: base(100, 100)
		{
			...
			AddHtml(280, 5, 210, 20, "<basefont size=2 color=#FFCC33>Moongate Library<basefont>", false, false);
		...
		public override void OnResponse(NetState state, RelayInfo info)
		{
            ...
            else if (Factions.Sigil.ExistsOn(pm))
            {
                m_Mobile.SendLocalizedMessage(1061632); // You can't do that while carrying the sigil.
            }
			else if (m_Mobile.Player && m_Mobile.Kills >= Mobile.MurderCount && entry.Map != Map.Felucca && !m_MoongateLibrary.m_RedsAnywhere)
			{
            ...
            else if (customRegion != null && customRegion.Controller.CannotEnter)
            {
                m_Mobile.SendLocalizedMessage(501942); // You are not allowed to travel there.
            }
            else if (region != null && region is GreenAcres && pm.AccessLevel < AccessLevel.GameMaster ) //Al: Block if GreenAcres
            {
                m_Mobile.SendLocalizedMessage(501942); // You are not allowed to travel there.
            }
            else if (!entry.Map.CanSpawnMobile(entry.Location.X, entry.Location.Y, entry.Location.Z))
            {
                m_Mobile.SendLocalizedMessage(501942); // That location is blocked.
            }
            else if ( SpellHelper.CheckMulti(entry.Location, entry.Map) )
            {
                m_Mobile.SendLocalizedMessage(501942); // That location is blocked.
            }
	...
	public class MoongateLibraryConfirmGump : Gump
	{
		...
		public override void OnResponse(NetState state, RelayInfo info)
		{
			if (info.ButtonID == 1)
			{
				...
				if (!m_Mobile.InRange(m_MoongateLibrary.GetWorldLocation(), 1) || m_Mobile.Map != m_MoongateLibrary.Map)
					m_Mobile.SendLocalizedMessage(1019002); // You are too far away to use the gate.
				else if (m_MoongateLibrary.Cost == 0 || myGate || CollectGold(m_Mobile, m_MoongateLibrary.Cost))
				{
					m_MoongateLibrary.AddToCollectedAmount(m_MoongateLibrary.Cost);
