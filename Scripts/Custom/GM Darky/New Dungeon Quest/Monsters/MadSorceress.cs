	...
	public class MadSorceress : BaseCreature
	{
		...
		public override void OnActionCombat()
		{
			...
			if ( DateTime.Now >= m_NextBomb )
			{
				...
				if ( 0.75 >= Utility.RandomDouble() && (m_Thrown % 2) == 1 ) // 75% chance to quickly throw another bomb
					m_NextBomb = DateTime.Now + TimeSpan.FromSeconds( 2.0 );
				else
					m_NextBomb = DateTime.Now + TimeSpan.FromSeconds( 4.0 + (9.0 * Utility.RandomDouble()) ); // 4-13 seconds
			}
		}
		...
		private class InternalTimer : Timer
		{
			...
			protected override void OnTick()
			{
				...
				AOS.Damage( m_Mobile, m_From, Utility.RandomMinMax( 35, 50 ), 0, 100, 0, 0, 0 );
			}
		}
