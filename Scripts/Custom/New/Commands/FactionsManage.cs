...
namespace Server.Gumps
{
	...
	public class FactionsManageGump : Gump
	{
		...
		{
			CommandSystem.Register( "FactionsManage", AccessLevel.Lead, new CommandEventHandler( FactionsManage_OnCommand ) );
		}
		...
		{
			if ( m_From.AccessLevel < AccessLevel.Lead )
				return;
			...
			switch ( info.ButtonID )
			{
				...
				case 24: // Faction 4
				{
					...
					List<Faction> factions = Faction.Factions;
					List<PlayerState> playerStates = factions[id].Members;
				...
				case 116: // Skills
				{
					...
					if ( player != null && !player.Deleted )
						m_From.SendGump( new Server.Gumps.SkillsGump( m_From, player ) );
					break;
				}
