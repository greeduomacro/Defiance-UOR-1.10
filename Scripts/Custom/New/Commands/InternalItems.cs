		...
		public static ArrayList BuildList( Mobile owner )
		{
			...
			foreach ( Item i in World.Items.Values )
			{
				if (i is CommodityDeed && ((CommodityDeed)i).Commodity != null)
					commodities.Add(i);
				else if (i.Map == Map.Internal && i.Parent == null && !(i is Server.Items.Fists) && !(i is MountItem) && !(i is Server.Items.EffectItem) )
					list.Add(i);
			}
			foreach ( CommodityDeed c in commodities )
				list.Remove( c.Commodity );
