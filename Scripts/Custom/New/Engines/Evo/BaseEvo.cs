...
namespace Xanthos.Evo
{
	public interface IEvoCreature
	{
		...
		bool HasEgg{ get; set; }
		BaseEvoSpec GetEvoSpec();
		Type GetEvoDustType();
		int Ep{ get; set; }
		int FinalStage{ get; }
		int NextEpThreshold{ get; set; }
		void Evolve( bool hatching );
		int Stage{ get; }
	}
	...
	{
		private static double kOverLimitLossChance = 0.00;	// Chance that loyalty will be lost if over followers limit
		...
		public BaseEvo( string name ) : base( AIType.AI_Mage, FightMode.Closest, 10, 1, 0.2, 0.4 )
		{
			Name = name;
			Init();
		}
		...
		{
                        // jakob added this if.
		        if (this.m_Stage < 3 || this.Region is DungeonRegion || this.Region is CustomRegion || SpellHelper.IsFeluccaDungeon( Map, Location ) )
		        {
		...
		private void MatingTarget_Callback( Mobile from, object obj )
		{
			...
			else
			{
				...
				else
				{
					...
					from.SendMessage( "You ask the owner of the pet if they will let your female mate with their male." );
				}
			}
		}
