...
namespace Arya.Jail
{
	...
	public class JailEntry
	{
		...
		public bool Expired
		{
			get
			{
				if (!m_AutoRelease) return false;
				try
				{
					return (m_JailTime + m_Duration) <= DateTime.Now;
				}
				catch
				{
		...
		{
			m_Comments.Add( string.Format( "[{0}] {1}: {2}", DateTime.Now.ToShortDateString(), from.Name, comment ) );
		}
