...
namespace Server.Engines.BulkOrders
{
	...
	public abstract class LargeBOD : Item
	{
		...
		public override void GetProperties( ObjectPropertyList list )
		{
            ...
            bool largehuntbod = (this is LargeHuntBOD);
            ...
            for ( int i = 0; i < m_Entries.Length; ++i )
                if ( largehuntbod )
                    list.Add( 1060658 + i, "{0}\t{1}", m_Entries[i].Details.Type.Name, m_Entries[i].Amount ); // ~1_val~: ~2_val~
                else
		...
		public void EndCombine( Mobile from, object o )
		{
			if ( o is Item && ((Item)o).IsChildOf( from.Backpack ) )
			{
				if ( o is SmallBOD )
				{
                    ...
                    else if ( !largehuntbod && m_Material >= BulkMaterialType.DullCopper && m_Material <= BulkMaterialType.Valorite && small.Material != m_Material )
                    {
