...
namespace Server.Engines.BulkOrders
{
	...
	public abstract class SmallBOD : Item
	{
		...
		[Constructable]
		public SmallBOD( int hue, int amountMax, Type type, int number, int graphic, bool requireExeptional, BulkMaterialType material, Mobile m ) : base( Core.AOS ? 0x2258 : 0x14EF )
		{
			Weight = 1.0;
		...
		public void BeginCombine( Mobile from )
		{
			...
			{
				// ***
				bool bUsesCreatures = (this is SmallHuntBOD);

				if ( bUsesCreatures )
				{
					from.SendMessage( "Target the corpse to add." );
					from.Target = new HuntBodUtility.HuntBodTarget( this );
				}
				else
					// ***
