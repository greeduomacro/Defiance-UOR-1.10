...
namespace Server.Engines.CannedEvil
{
	public class ChampionCommands
	{
		...
		{
			CommandSystem.Register( "ClearChampByTarget", AccessLevel.Lead, new CommandEventHandler( KillByTarget_OnCommand ) );
			CommandSystem.Register( "ClearChampByRegion", AccessLevel.Lead, new CommandEventHandler( KillByRegion_OnCommand ) );
		}
		...
		private class KillTarget : Target
		{
			...
			{
				if ( from == null || from.AccessLevel < AccessLevel.Lead )
					return;
		...
		{
			ChampionSpawnRegion region = e.Mobile.Region as ChampionSpawnRegion;
	...
	public class ChampionSpawn : Item
	{
		...
		private bool m_ReadyToActivate;
		private bool m_RandomizeType;
		private bool m_ActivatedByValor;
		...
		public virtual string BroadcastMessage{ get{ return "The Champion of this region has sensed your presence!  Beware its wrath!"; } }
		public virtual bool ProximitySpawn{ get{ return false; } }
		public virtual bool CanAdvanceByValor{ get{ return true; } }
		public virtual bool CanActivateByValor{ get{ return true; } }
		public virtual bool AlwaysActive{ get{ return true; } }
		...
		public ChampionSpawn() : base( 0xBD2 )
		{
			...
			m_ExpireDelay = TimeSpan.FromMinutes( 30.0 );
			m_RestartDelay = TimeSpan.FromMinutes( 30.0 );
		...
		public bool ReadyToActivate
		{
			get
			{
				return m_ReadyToActivate;
			}
			set
			{
				m_ReadyToActivate = value;
			}
		}

		[CommandProperty( AccessLevel.GameMaster )]
		public bool ActivatedByValor
		{
			get
			{
				return m_ActivatedByValor;
			}
			set
			{
				m_ActivatedByValor = value;
			}
		}
		...
		public void Stop()
		{
			...
			Timer.DelayCall( TimeSpan.FromMinutes( 10.0 ), new TimerCallback( ExpireCreatures ) );
		}
		...
		public static void GiveScrollOfTranscendenceFelTo ( Mobile killer, ScrollofTranscendence SoTF )
		{
			...
			for (int j = 0; j < pm.JusticeProtectors.Count; ++j)
			{
				...
				if (prot.Map != killer.Map || prot.Kills >= Mobile.MurderCount || prot.Criminal || !JusticeVirtue.CheckMapRegion(killer, prot))
				continue;
		...
		{
			if ( !m_Active && !m_ReadyToActivate && !AlwaysActive )
				DeleteCreatures();
		}
		...
		public override void Serialize( GenericWriter writer )
		{
			...
			writer.Write( (bool)m_ActivatedByValor );
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			switch( version )
			{
				case 6:
				{
					m_ActivatedByValor = reader.ReadBool();
					goto case 5;
				}
				case 5:
				{
				...
				case 0:
				{
					...
					for ( int i = 0;i < m_Creatures.Count; i++ )
						if ( m_Creatures[i] is BaseCreature )
							((BaseCreature)m_Creatures[i]).ChampSpawn = this;
					break;
				}
			}
	...
	public class ChampionSpawnRegion : BaseRegion
	{
		...
		{
			if ( m.Player && m.AccessLevel == AccessLevel.Player && !m_Spawn.Active )
			{
				Region parent = Parent;
				if ( parent == null )
					parent = this;

				if ( m_Spawn.ReadyToActivate )
					m_Spawn.Start();
				else if ( m_Spawn.ProximitySpawn )
				{
					List<Mobile> players = parent.GetPlayers( true );
					List<IPAddress> addresses = new List<IPAddress>();
					for ( int i = 0; i < players.Count; i++ )
						if ( players[i].NetState != null && !addresses.Contains( players[i].NetState.Address ) )
							addresses.Add( players[i].NetState.Address );

					if ( addresses.Count >= 15 )
					{
						foreach ( Mobile player in players )
							player.SendMessage( 0x20, m_Spawn.BroadcastMessage );

						m_Spawn.BeginRestart( m_Spawn.RestartDelay );
					}
				}
			}
		}
