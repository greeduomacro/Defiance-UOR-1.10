...
namespace Server.Engines.Craft
{
	public class CraftGump : Gump
	{
		...
		private static readonly ClientVersion m_ResourceCountClient = new ClientVersion( "6.0.0.0" );
		...
		public void CreateMakeLastList()
		{
			...
			if ( items.Count > 0 )
			{
				for ( int i = 0; i < items.Count; ++i )
				{
                    ...
                    // jakob, this will check the (bool whatever(Mobile)) method if you specified one
					if ( craftItem.Conditional == null || craftItem.Conditional( m_From ) )
					{
