...
namespace Server.Engines.Craft
{
	...
	public delegate bool ConditionalAvailibility( Mobile m );
	...
	public class CraftItem
	{
		...
		private ConditionalAvailibility m_Conditional;
		public ConditionalAvailibility Conditional { get { return m_Conditional; } set { m_Conditional = value; } }
		...
		public void Craft( Mobile from, CraftSystem craftSystem, Type typeRes, BaseTool tool )
		{
			...
			{
				if( (m_Conditional == null || m_Conditional( from )) && (RequiredExpansion == Expansion.None || ( from.NetState != null && from.NetState.SupportsExpansion( RequiredExpansion ) )) )
				{
			...
			if ( CheckSkills( from, typeRes, craftSystem, ref ignored, ref allRequiredSkills ) )
			{
				...
				if ( item != null )
				{
                    ...
                    if ( tool.Parent is Container )
						((Container)tool.Parent).AddItem( item );
                    else
						...
						if ( faction != null )
						{
							...
							if ( town != null && town.Owner == faction )
							{
								...
								if ( pack != null )
								{
									...
									{
										CraftShopRegion region = from.Region as CraftShopRegion;
										queryFactionImbue = ( region != null && region.HasSkill( def.Skills ) ) || ( def.Skills == CraftSkillType.None && Faction.IsNearType( from, def.VendorType, 12 ) );
									}
								}
							}
						}
                ...
                if ( queryFactionImbue )
                {
                    if ( item is SpellScroll )
                    {
                        ...
                        // Spell ID's
                        // : Heal = 3, Harm = 11
                        // : Fireball = 17, Greater Heal = 28
                        // : Lightning = 29
						if ( scroll.SpellID == 3 || scroll.SpellID == 11 || scroll.SpellID == 17 || scroll.SpellID == 28 || scroll.SpellID == 29 )
