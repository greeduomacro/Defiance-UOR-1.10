...
/*
			// jakob, added bola creation
			index = AddCraft( typeof( Bola ), 1044050, 1046441, 90.0, 110.0, typeof( IronIngot ), 1044036, 3, 1044037 );
			AddRes( index, typeof( Silver ), 1017384, 250, 1042204 );
			AddRes( index, typeof( Gold ), 3000083, 500, 1042205 );
			SetHasConditional( index, new ConditionalAvailibility( IsFactionMember ) );
			// end
*/
			// jakob, added special item creation
			// *** SPECIAL ITEM CREATION ***

			//Type specialType = typeof( ElvenRobe );
			//int specialCliloc = 1041350;
/*
			int silverAmount = 10000;

			Type resourceType = typeof( AncientSigil );
			int resourceCliloc = 1041350;

			index = AddCraft( specialType, 1044050, specialCliloc, 100.0, 100.0, typeof( Silver ), 1017384, silverAmount, 1042204 );
			AddRes( index, resourceType, resourceCliloc, 20, 502925 );
			SetHasConditional( index, new ConditionalAvailibility( HasBritainMaginciaAndYew ) );
*/
			// *** SPECIAL ITEM CREATION ENDS HERE ***
			//end
			#endregion
			...
			#region Traps
			// Dart Trap
			index = AddCraft( typeof( DartTrapCraft ), 1044052, 1024396, 55.0, 115.0, typeof( IronIngot ), 1044036, 10, 1044037 );
			AddRes( index, typeof( Bolt ), 1044570, 10, 1044253 );

			// Poison Trap
			index = AddCraft( typeof( LesserPoisonTinkerTrap ), 1044052, "lesser poison tinker trap", 50.0, 75.0, typeof( IronIngot ), 1044036, 10, 1044037 );
			AddRes( index, typeof( LesserPoisonPotion ), 1044548, 10, 1044253 );

			index = AddCraft( typeof( PoisonTinkerTrap ), 1044052, "poison tinker trap", 60.0, 95.0, typeof( IronIngot ), 1044036, 10, 1044037 );
			AddRes( index, typeof( PoisonPotion ), 1044549, 10, 1044253 );

			index = AddCraft( typeof( GreaterPoisonTinkerTrap ), 1044052, "greater poison tinker trap", 80.0, 99.0, typeof( IronIngot ), 1044036, 10, 1044037 );
			AddRes( index, typeof( GreaterPoisonPotion ), 1044550, 10, 1044253 );

			index = AddCraft( typeof( DeadlyPoisonTinkerTrap ), 1044052, "deadly poison tinker trap", 100.0, 125.0, typeof( IronIngot ), 1044036, 10, 1044037 );
			AddRes( index, typeof( DeadlyPoisonPotion ), 1044551, 10, 1044253 );

			// Explosion Trap
			index = AddCraft( typeof( LesserExplosionTinkerTrap ), 1044052, "lesser explosion tinker trap", 50.0, 95.0, typeof( IronIngot ), 1044036, 10, 1044037 );
			AddRes( index, typeof( LesserExplosionPotion ), 1044555, 10, 1044253 );

			index = AddCraft( typeof( ExplosionTinkerTrap ), 1044052, "explosion tinker trap", 70.0, 99.0, typeof( IronIngot ), 1044036, 10, 1044037 );
			AddRes( index, typeof( ExplosionPotion ), 1044556, 10, 1044253 );

			index = AddCraft( typeof( GreaterExplosionTinkerTrap ), 1044052, "greater explosion tinker trap", 100.0, 125.0, typeof( IronIngot ), 1044036, 10, 1044037 );
			AddRes( index, typeof( GreaterExplosionPotion ), 1044557, 10, 1044253 );
			...
			AddRes( index, typeof( DeadlyPoisonPotion ), 1044571, 10, 1044253 );
			...
			AddRes( index, typeof( IronIngot ), 1044036, 10, 1044037 );
			AddRes( index, typeof( GreaterExplosionPotion ), 1044569, 10, 1044253 );
		...
		// jakob, added these to check for things
		private static bool IsFactionMember( Mobile m )
		{
			return Faction.Find( m ) != null;
		}

		private static bool HasBritainMaginciaAndYew( Mobile m )
		{
			Faction faction = Faction.Find( m );
			if ( faction == null )
				return false;
			else
				return Town.Parse("Britain").Owner == faction && Town.Parse("Magincia").Owner == faction && Town.Parse("Yew").Owner == faction;
		}
		// end
	}
	...
	public abstract class TrapCraft : CustomCraft
	{
		...
		public override Item CompleteCraft( out int message )
		{
			...
			if ( message == 0 )
			{
				int trapLevel = (int)(From.Skills.Tinkering.Value / 20);
