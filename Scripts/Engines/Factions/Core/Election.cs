...
namespace Server.Factions
{
	...
	{
		public static readonly TimeSpan PendingPeriod = TimeSpan.FromDays( 10.0 );
		public static readonly TimeSpan CampaignPeriod = TimeSpan.FromDays( 2.0 );
	...
	public class Candidate
	{
		...
		public int Votes
		{
			get
			{
				...
				foreach ( Voter voter in m_Voters )
				{
					PlayerState ps = PlayerState.Find( voter.From );

					if ( ps != null )
					{
						TimeSpan timeDiff = DateTime.Now - ps.JoinDate;
						if ( timeDiff.TotalDays >= 90.0 )
							votes += 3;
						else if ( timeDiff.TotalDays >= 60.0 )
							votes += 2;
						else
							votes += 1;
					}
