...
namespace Server.Factions
{
	...
	public abstract class Faction : IComparable
	{
		...
		public static bool AlreadyHasCharInFaction( Mobile mob )
		{
			Account acct = mob.Account as Account;
			...
			else if ( pm.Guild != null )
			{
				...
				else
				{
					...
					for ( int i = 0; i < members.Count; i++ )
					{
						...
						if ( member != null && !HasEnoughSkills( member ) )
						{
		...
		public static void CleanUpFactions()
		{
			...
			for ( int i = 0; i < factions.Count; i++ )
			{
				...
				List<PlayerState> list = new List<PlayerState>( f.Members );

				for ( int j = 0; j < list.Count; j++ )
				{
					PlayerState ps = list[j];
					Mobile m = ps.Mobile;

                    if ( ( DateTime.Now - ps.JoinDate ) > MustKillThisOften && ( DateTime.Now - ps.LastKill ) > MustKillThisOften && !MerchantTitles.HasMerchantQualifications( m ) )
					{
						f.RemoveMember( m );
						count ++;
					}
		...
		public const int StabilityActivation = 1000; // Stablity code goes into effect when largest faction has > 200 people

		private static int GetMemberCount( Faction faction )
		{
			List<Account> accountlist = new List<Account>();

			foreach (PlayerState ps in faction.Members)
			{
				Account acc = (Account)ps.Mobile.Account;
		...
		public bool CanHandleInflux( int influx )
		{
			...
			if ( StabilityFactor > 0 && (((GetMemberCount(this) + influx) * 100) / StabilityFactor) > GetMemberCount(smallest))
				return false;
		...
		{
			if ((victim != null && victim.Region is CustomRegion && ((CustomRegion)victim.Region).Controller.NoFactionEffects) ||
				(killer != null && killer.Region is CustomRegion && ((CustomRegion)killer.Region).Controller.NoFactionEffects))
					return;
			...
			if ( pack != null )
			{
				...
				for ( int i = 0; i < sigils.Length; ++i )
				{
					...
					if ( killerState != null && killerPack != null )
					{
						...
						else if ( killer.Region.IsPartOf( typeof( HouseRegion ) ) )
						{
							killer.Location = ((HouseRegion)(killer.Region)).House.BanLocation;
							killer.SendMessage( "You may not carry sigils inside a house" );
						}
					}
					else
					{
			...
			if ( victim is BaseCreature && !(victim is BaseVendor) )
			{
                ...
                bool bcRegionIsNoFactions = (bc.Region is CustomRegion && ((CustomRegion)bc.Region).Controller.NoFactionEffects);

                if ( Faction.IsFactionFacet(bc.Map) && victimFaction != null && killerState.Faction != victimFaction && !bcRegionIsNoFactions )
				{
