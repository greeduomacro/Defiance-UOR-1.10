...
namespace Server.Factions
{
	public class Sigil : BaseSystemController
	{
		...
		public static readonly TimeSpan CorruptionGrace = TimeSpan.FromMinutes( (Core.SE) ? 30.0 : 60.0 );

		// Sigil must be held at a stronghold for this amount of time in order to become corrupted
		public static readonly TimeSpan CorruptionPeriod = ( (Core.SE) ? TimeSpan.FromHours( 10.0 ) : TimeSpan.FromHours( 10.0 ) );

		// After a sigil has been corrupted it must be returned to the town within this period of time
		public static readonly TimeSpan ReturnPeriod = TimeSpan.FromMinutes( 60.0 );

		// Once it's been returned the corrupting faction owns the town for this period of time
		public static readonly TimeSpan PurificationPeriod = TimeSpan.FromDays( 1.5 );
		// end
		...
		public override void OnSingleClick( Mobile from )
		{
			...
			if ( IsCorrupting )
			{
				double hours = ( DateTime.Now - m_CorruptionStart ).TotalHours; //Should be less than or equal to CorruptPeriod.
				double ratio = hours / CorruptionPeriod.TotalHours;

				if ( ratio >= 0.80 )
					LabelTo( from, "This sigil is nearing complete corruption." );
				else if ( ratio >= 0.60 )
					LabelTo( from, "This sigil appears heavily corrupted." );
				else if ( ratio >= 0.40 )
					LabelTo( from, "This sigil appears moderately corrupted." );
				else if ( ratio >= 0.20 )
					LabelTo( from, "This sigil appears slightly corrupted." );
				else
					LabelTo( from, "This sigil has just started the corruption process." );
			...
			if ( obj is Mobile )
			{
				if ( obj is PlayerMobile )
				{
					...
					if ( toFaction == null )
						...
						from.SendMessage( "You cannot give this sigil to someone inside of a house!" );
			...
			else if ( obj is BaseMonolith )
			{
				...
				if ( obj is StrongholdMonolith )
				{
					...
					else
					{
						...
						// jakob, award point for steal
						PlayerState ps = PlayerState.Find( from );
						if ( ps != null )
							ps.StolenSigilsCount++;
						// end
					}
				}
				#endregion
				...
				else if ( obj is TownMonolith )
				{
					...
					// jakob, make sure they have all control points
					else if ( !m.HasAllControlPoints( m_Corrupted ) )
						from.SendMessage( "You cannot capture this town unless your faction controls all the control points." );
					// end
					else
					{
						m.Sigil = this;
