	...
	public class Obsidian : Item
	{
		...
		private class DisassembleEntry : ContextMenuEntry
		{
			...
			public override void OnClick()
			{
				...
				if ( !m_Obsidian.Deleted && m_Obsidian.Quantity >= Obsidian.Partial && m_Obsidian.Quantity < Obsidian.Completed && m_Obsidian.IsChildOf( from.Backpack ) && from.CheckAlive() )
				{
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			switch ( version )
			{
				...
				case 0:
				{
					string statuename = reader.ReadString();
					for ( int i = 0; i < m_Names.Length; i++ )
					{
						if ( statuename == m_Names[i] )
						{
							m_StatueName = i;
							return;
						}
