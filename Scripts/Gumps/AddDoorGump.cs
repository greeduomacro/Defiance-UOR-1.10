	...
	public class AddDoorGump : Gump
	{
		...
		public AddDoorGump( int type ) : base( 50, 40 )
		{
			...
			if ( m_Type >= 0 && m_Type < m_Types.Length )
			{
				...
				int baseID = m_Types[m_Type].BaseID;
			...
			else
			{
				...
				for ( int i = 0; i < m_Types.Length; ++i )
				{
					...
					AddItem( 22 + (i * 49), 20, m_Types[i].BaseID );
				}
			}
		}
		...
		public override void OnResponse( NetState sender, RelayInfo info )
		{
			...
			else
			{
				if ( button >= 0 && button < 8 )
				{
					...
					CommandSystem.Handle( from, String.Format( "{0}Add {1} {2}", CommandSystem.Prefix, m_Types[m_Type].Type.Name, (DoorFacing) button ) );
				}
				else if ( button == 8 )
				{
					from.SendGump( new AddDoorGump( m_Type ) );
