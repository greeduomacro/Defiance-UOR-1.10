...
namespace Server.Gumps
{
	...
	public class AdminGump : Gump
	{
			...
			switch ( pageType )
			{
				...
				case AdminGumpPage.Clients:
				{
					...
					for ( int i = 0, index = (listPage * 12); i < 12 && index >= 0 && index < m_List.Count; ++i, ++index )
					{
						...
						AddLabelCropped(  95, offset, 81, 20, DonationSystem.HasDonated( a ) ? 54 : LabelHue, a == null ? "(no account)" : a.Username );
				...
				case AdminGumpPage.Accounts:
				{
					...
					for ( int i = 0, index = (listPage * 12); i < 12 && index >= 0 && index < m_List.Count; ++i, ++index )
					{
						...
						if ( rads == null )
						{
							AddLabelCropped( 12, offset, 120, 20, DonationSystem.HasDonated( a ) ? 54 : LabelHue, a.Username );
						}
						else
						{
							AddCheck( 10, offset, 0xD2, 0xD3, rads.Contains( a ), index );
							AddLabelCropped( 32, offset, 100, 20, DonationSystem.HasDonated( a ) ? 54 : LabelHue, a.Username );
						}
				...
				case AdminGumpPage.FirewallInfo:
				{
					...
					if ( m_List == null )
					{
						...
						foreach ( Account acct in Accounts.GetAccounts() )
						{
							...
							for( int i = 0; !contains && i < acct.LoginIPs.Count; ++i )
							{
								if( ((Firewall.IFirewallEntry)state).IsBlocked( acct.LoginIPs[i] ) )
								{
									m_List.Add( acct );
									break;
								}
		...
		private static ArrayList GetAllSharedAccounts()
		{
			...
			foreach ( Account acct in Accounts.GetAccounts() )
			{
				List<IPAddress> theirAddresses = acct.LoginIPs;

				for ( int i = 0; i < theirAddresses.Count; ++i )
				{
		...
		private static ArrayList GetSharedAccounts( List<IPAddress> ipAddresses )
		{
			ArrayList list = new ArrayList();
			...
			foreach ( Account acct in Accounts.GetAccounts() )
			{
				...
				for ( int i = 0; !contains && i < theirAddresses.Count; ++i )
				{
					...
					for ( int j = 0; !contains && j < ipAddresses.Count; ++j )
		...
		public static void FirewallShared_Callback( Mobile from, bool okay, object state )
		{
			...
			if ( okay )
			{
				...
				for ( int i = 0; i < a.LoginIPs.Count; ++i )
				...
				case 5:
				{
					switch ( index )
					{
						...
						case 18: // firewall all
						{
							...
							{
								from.SendGump( new WarningGump( 1060635, 30720, String.Format( "You are about to firewall {0} address{1}. Do you wish to continue?", a.LoginIPs.Count, a.LoginIPs.Count != 1 ? "s" : "" ), 0xFFC000, 420, 400, new WarningGumpCallback( FirewallShared_Callback ), a ) );
							}
							else
							{
								from.SendGump( new AdminGump( from, AdminGumpPage.AccountDetails_Access_ClientIPs, 0, null, "This account has not yet been accessed.", m_State ) );
						...
						case 31: // View all inactive accounts
						{
							...
							foreach ( Account acct in Accounts.GetAccounts() )
							{
								if ( acct.Inactive && !DonationSystem.HasDonated( acct ) )
									results.Add( acct );
							}
