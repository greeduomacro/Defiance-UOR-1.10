...
namespace Server.Gumps
{
	public class GoGump : Gump
	{
		...
		public static void DisplayTo( Mobile from, Mobile to )
		{
			LocationTree tree;

			if ( to.Map == Map.Ilshenar )
				tree = Ilshenar;
			else if ( to.Map == Map.Felucca )
				tree = Felucca;
			else if ( to.Map == Map.Trammel )
				tree = Trammel;
			else if ( to.Map == Map.Malas )
				tree = Malas;
			else if ( to.Map == Map.Tokuno )
				tree = Tokuno;
			else
				tree = TerMur;

			ParentNode branch = null;
			tree.LastBranch.TryGetValue( to, out branch );

			if ( branch == null )
				branch = tree.Root;

			if ( branch != null )
				from.SendGump( new GoGump( 0, from, tree, branch, to ) );
		}
		...
		private GoGump( int page, Mobile from, LocationTree tree, ParentNode node, Mobile to ) : base( 50, 50 )
		{
			from.CloseGump( typeof( GoGump ) );

			tree.LastBranch[to] = node;
			...
			switch ( info.ButtonID )
			{
				case 1:
				{
					if ( m_Node.Parent != null )
						from.SendGump( new GoGump( 0, from, m_Tree, m_Node.Parent, m_To ) );
				...
				{
					if ( m_Page > 0 )
						from.SendGump( new GoGump( m_Page - 1, from, m_Tree, m_Node, m_To ) );
				...
				default:
				{
					...
					if ( index >= 0 && index < m_Node.Children.Length )
					{
						...
						if ( o is ParentNode )
						{
							from.SendGump( new GoGump( 0, from, m_Tree, (ParentNode)o, m_To ) );
						}
						else
						{
							ChildNode n = (ChildNode)o;

							m_To.MoveToWorld( n.Location, m_Tree.Map );
						}
					}
