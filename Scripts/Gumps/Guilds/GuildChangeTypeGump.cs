...
namespace Server.Gumps
{
	public class GuildChangeTypeGump : Gump
	{
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			if ( pl != null && m_Guild.TypeLastChange.AddDays( 7 ) <= DateTime.Now )
			{
				Faction faction = pl.Faction;

				Faction com = CouncilOfMages.Instance;
				Faction tb = TrueBritannians.Instance;
				Faction sl = Shadowlords.Instance;
				Faction minax = Minax.Instance;

				if ( ((faction == com || faction == tb) && newType == GuildType.Order) || ((faction == sl || faction == minax) && newType == GuildType.Chaos) )
					okToChange = true;
				else
