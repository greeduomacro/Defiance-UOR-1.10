					...
					switch ( index )
					{
						...
						case 12: // Make Private
						{
							if ( isOwner )
							{
								...
								else if ( m_House.VendorRentalContracts.Count > 0 )
								{
								...
								else if ( th != null && th.ForSaleSign.ForcePublic && th.Public )
									from.SendGump( new NoticeGump( 1060637, 30720, "This house cannot be private.", 32512, 320, 180, new NoticeGumpCallback( PublicPrivateNotice_Callback ), m_House ) );
								else
								{
						...
						case 13: // Make Public
						{
							if ( isOwner )
							{
								if ( th != null && th.ForSaleSign.ForcePrivate && !th.Public )
									from.SendGump( new NoticeGump( 1060637, 30720, "This house cannot be public.", 32512, 320, 180, new NoticeGumpCallback( PublicPrivateNotice_Callback ), m_House ) );
								else
								{
									m_House.Public = true;
					...
					switch ( index )
					{
						case 0: // Demolish
						{
							if ( isOwner && m_House.MovingCrate == null && m_House.InternalizedVendors.Count == 0 )
							{
								...
								{
									from.CloseGump( typeof( ConfirmCommitGump ) );
						...
						case 1: // Trade House
						{
							if ( isOwner )
							{
								...
								else if ( th != null && th.ForSaleSign.NoTrade )
									 from.SendMessage("This house cannot be traded.");
								else
								{
