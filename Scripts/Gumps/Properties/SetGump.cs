	...
	public class SetGump : Gump
	{
		...
		private class InternalPicker : HuePicker
		{
			...
			public override void OnResponse( int hue )
			{
				try
				{
					...
					CommandLogging.LogChangeProperty( m_Mobile, m_Object, m_Property.Name, getval == null ? "(null)" : getval.ToString(), hue.ToString() );
		...
		public override void OnResponse( NetState sender, RelayInfo info )
		{
			...
			if ( shouldSet )
			{
				...
				{
					object getval = m_Property.GetValue( m_Object, null );
					CommandLogging.LogChangeProperty( m_Mobile, m_Object, m_Property.Name, getval == null ? "(null)" : getval.ToString(), toSet==null?"(null)":toSet.ToString() );
