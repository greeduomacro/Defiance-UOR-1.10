...
{
	public abstract class BaseArmor : Item, IScissorable, IFactionItem, ICraftable, IWearableDurability, IAosAttributes, IEthicsItem
	{
		#region Ethics
		...
		public override Type DyeType
		{
			get
			{
				switch ( MaterialType )
				{
					default: return typeof(DyeTub);
					case AMT.Plate: case AMT.Chainmail: case AMT.Ringmail: return typeof(RewardMetalDyeTub);
					case AMT.Leather: case AMT.Studded: case AMT.Barbed: case AMT.Spined: case AMT.Horned: return typeof(LeatherDyeTub);
				}
			}
		}
		...
		{
			if ( m_Resource == CraftResource.None || ( m_Resource > CraftResource.RegularWood && m_Resource <= CraftResource.Frostwood ) )
				return false;
		...
		{
			double HalfAr = ArmorRatingScaled( attacker ) / 2.0;
