...
namespace Server.Items
{
	public class Bola : Item
	{
		...
		private static void FinishThrow( object state )
		{
			...
			if ( mt is FactionWarHorse && 0.6 < Utility.RandomDouble() )
			{
				from.SendMessage( "Their mount evades the bola." );
				to.SendMessage( "Your mount succesfully evaded the bola attempt!" );
				return;
			}
