...
*/
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			if ( version < 1 )
			{
				...
				int count = reader.ReadInt(); //KeyCount
				for ( int i = 0; i < count; i++ )
				{
					reader.ReadInt(); // version
					Key key = new Key();

					key.KeyValue = reader.ReadUInt();
					key.Description = reader.ReadString();
					key.MaxRange = reader.ReadInt();
					key.Link = reader.ReadItem();
					key.ItemID = reader.ReadInt(); //KeyType
					Add( key ); //Add it to the ring.
				}
			}
			else
