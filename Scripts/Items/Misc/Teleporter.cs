...
namespace Server.Items
{
	...
	{
		private bool m_Active, m_Creatures, m_CombatCheck, m_AllowPets;
		...
		private Dictionary<Mobile,Timer> m_Timers = new Dictionary<Mobile,Timer>();
		...
		public virtual void StartTeleport( Mobile m )
		{
			if ( m_Delay == TimeSpan.Zero )
				...
				m_Timers.Add( m, Timer.DelayCall( m_Delay, new TimerStateCallback( DoTeleport_Callback ), m ) );
		}
		...
		private void DoTeleport_Callback( object state )
		{
			...
			DoTeleport( m );
			Timer timer = null;
			m_Timers.TryGetValue( m, out timer );
			if ( timer != null )
			{
				timer.Stop();
				m_Timers.Remove( m );
				...
				if ( sk == null || sk.Base < m_Required )
				{
					if ( m.BeginAction( this ) )
					{
						if ( m_MessageString != null )
							m.Send( new UnicodeMessage( Serial, ItemID, MessageType.Regular, 0x3B2, 3, m.Language, null, m_MessageString ) );
