...
namespace Server.Items
{
	public class BaseShield : BaseArmor
	{
		...
		public override double ArmorRating( Mobile attacker )
		{
			Mobile m = this.Parent as Mobile;
			double ar = base.ArmorRating( attacker );
		...
		public override int OnHit( BaseWeapon weapon, Mobile attacker, int damage )
		{
			if( Core.AOS )
			{
				...
				{
					double halfArmor = ArmorRating( attacker ) / 2.0;
			...
			else
			{
				...
				double ar = this.ArmorRating( attacker );
				...
				*/
				if( owner.CheckSkill( SkillName.Parry, chance ) )
				{
					...
					if( 0.33 < Utility.RandomDouble() ) // 33% chance to lower durability
					{
