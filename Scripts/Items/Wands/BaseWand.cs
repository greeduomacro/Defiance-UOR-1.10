...
namespace Server.Items
{
	...
	public abstract class BaseWand : BaseBashing
	{
		...
		public BaseWand( WandEffect effect, int minCharges, int maxCharges ) : base( Utility.Random( 0xDF2, 4 ) )
		{
			Weight = 1.0;
			Effect = effect;
		...
		public void Cast( Spell spell )
		{
			if ( CanUse( spell ) )
			{
				...
				spell.Caster.SendMessage( "You do not have the experience to wield the magical power of this item." );
		}
		...
		public virtual bool CanUse( Spell spell )
		{
			double minSkill, maxSkill;
			spell.GetCastSkills( out minSkill, out maxSkill );

			return spell.Caster.Skills[spell.CastSkill].Value >= minSkill;
		}
