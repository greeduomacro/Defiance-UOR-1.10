...
namespace Server.Items
{
	...
	public abstract class BaseWeapon : Item, IWeapon, IFactionItem, ICraftable, ISlayer, IDurability, IAosAttributes, IEthicsItem
	{
...
/*
        public string GetChargedAbilityName()
        {
            switch ( m_ChargedAbility )
            {
                case WeaponChargedAbility.Clumsiness: return "clumsiness";
                case WeaponChargedAbility.Feeblemindedness: return "feeblemindedness";
                case WeaponChargedAbility.Weakness: return "weakness";
                case WeaponChargedAbility.Burning: return "burning";
                case WeaponChargedAbility.Wounding: return "wounding";
                case WeaponChargedAbility.DragonsBreath: return "dragon's breath";
                case WeaponChargedAbility.DaemonsBreath: return "daemon's breath";
                case WeaponChargedAbility.Thunder: return "thunder";
                case WeaponChargedAbility.MagesBane: return "mage's bane";
                case WeaponChargedAbility.GhoulsTouch: return "ghoul's touch";
                case WeaponChargedAbility.Evil: return "evil";
...
//		private int[] m_DiceDamage;
		...
		//public virtual int DiceDamage{ get{ return Utility.Dice( 1, 1, 0 ); } }
		#endregion
...
/*
		...
		{
			get{ return m_DiceDamage[0]; }
			set{ m_DiceDamage[0] = value; }
		}

		public int DamageDiceSides
		{
			get{ return m_DiceDamage[1]; }
			set{ m_DiceDamage[1] = value; }
		}

		public int DamageDiceBonus
		{
			get{ return m_DiceDamage[2]; }
			set{ m_DiceDamage[2] = value; }
		}
*/
		[CommandProperty( AccessLevel.GameMaster )]
		public float Speed
...
*/
			bool acidblood = (MaxRange <= 1 && (defender is Slime || defender is ToxicElemental /*|| defender is AcidElemental*/));

			if ( m_MaxHits > 0 && ( acidblood || Utility.Random( 25 ) == 0) ) // Stratics says 50% chance, seems more like 4%..
			{
				if ( acidblood )
...
*/
		...
		public virtual void DoMagicArrow( Mobile attacker, Mobile defender )
		{
			...
			double damage = GetAosDamage( attacker, 3, 1, 10 );
		...
		public virtual void DoHarm( Mobile attacker, Mobile defender )
		{
			...
			double damage = GetAosDamage( attacker, 6, 3, 7 );
		...
		public virtual void DoFireball( Mobile attacker, Mobile defender )
		{
			...
			double damage = GetAosDamage( attacker, 6, 3, 6 );
		...
		public virtual void DoLightning( Mobile attacker, Mobile defender )
		{
			...
			double damage = GetAosDamage( attacker, 6, 3, 5 );
...
/*
		public virtual void AddBlood( Mobile attacker, Mobile defender, int damage )
		{
			...
			for ( int i = 0; i < bloodcount; i++ )
				CreateBlood( defender.Location, defender.Map, blood, i > 3 );
		}

		public static void CreateBlood( Point3D loc, Map map, int blood, bool delayed )
		{
			new Blood( blood, delayed ).MoveToWorld( new Point3D( BloodOffset( loc.X ), BloodOffset( loc.Y ), loc.Z ), map );
		}
...
//				damageBonus = 100;
			...
			double totalBonus = strengthBonus + anatomyBonus + tacticsBonus + lumberBonus + ((double)(GetDamageBonus( attacker ) + damageBonus) / 100.0);
...
//			SetSaveFlag( ref flags2, SaveFlag2.DiceDamage,		m_DiceDamage[0] != -1 && m_DiceDamage[1] != -1 && m_DiceDamage[2] != -1 );
...
/*			if( GetSaveFlag( flags2, SaveFlag2.DiceDamage ) )
			{
				writer.WriteEncodedInt( (int) m_DiceDamage[0] );
				writer.WriteEncodedInt( (int) m_DiceDamage[1] );
				writer.WriteEncodedInt( (int) m_DiceDamage[2] );
			} */
...
/*
			if( GetSaveFlag( flags, SaveFlag.DamageDice ) )
			{
				writer.WriteEncodedInt( (int) m_DiceDamage[0] );
				writer.WriteEncodedInt( (int) m_DiceDamage[1] );
				writer.WriteEncodedInt( (int) m_DiceDamage[2] );
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			switch ( version )
			{
				...
				case 8:
				{
					if ( version < 9 )
					{
						/*m_ChargedAbility = (WeaponChargedAbility)*/reader.ReadInt();
...
*/
		...
		public override void GetProperties( ObjectPropertyList list )
		{
			...
			if ( (prop = (GetDamageBonus( null ) + m_AosAttributes.WeaponDamage)) != 0 )
			...
			if ( (prop = (GetHitChanceBonus( null ) + m_AosAttributes.AttackChance)) != 0 )
		...
		public override void OnSingleClick( Mobile from )
		{
			...
			if ( !m_Identified || attrs.Count > 0 || Crafter != null || String.IsNullOrEmpty( Name ) )
			{
				EquipmentInfo eqInfo = new EquipmentInfo( number, m_Crafter, !m_Identified, attrs.ToArray() );
