...
*/
			...
			PlaceItemIn( cont, 117, 115, new TheBerserkersMaul() );
...
*/
		...
		private static void EventSink_CharacterCreated( CharacterCreatedEventArgs args )
		{
			...
			CityInfo city = new CityInfo( "Young Town", "Young Town Entrance", 1243, 1193, -29, Map.Ilshenar );
		...
		private static bool SetName( Mobile m, string name )
		{
			name = name.Trim();

			if ( NameVerification.ValidatePlayerName( name, 2, 16, true, false, true, 1, NameVerification.SpaceDashPeriodQuote ) != NameResultMessage.Allowed )
				m.Name = String.Format( "Generic Player {0}", Utility.Random( 100, 25000 ) );
...
*/
					...
					Spellbook book = new Spellbook( (ulong)UInt64.MaxValue );
