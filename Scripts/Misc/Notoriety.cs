	...
	public class NotorietyHandlers
	{
		...
		public static int CorpseNotoriety( Mobile source, Corpse target )
		{
			...
			if( cretOwner != null )
			{
				...
				bool srcRegionIsNoFactions = (source.Region is CustomRegion && ((CustomRegion)source.Region).Controller.NoFactionEffects);

				if( srcFaction != null && trgFaction != null && srcFaction != trgFaction && Faction.IsFactionFacet( source.Map ) && !srcRegionIsNoFactions )
					return Notoriety.Enemy;
				...
				if( target.Kills >= Mobile.MurderCount || (body.IsMonster && IsSummoned( target.Owner as BaseCreature )) || (target.Owner is BaseCreature && (((BaseCreature)target.Owner).AlwaysMurderer || ((BaseCreature)target.Owner).IsAnimatedDead)) )
		...
		public static int MobileNotoriety( Mobile source, Mobile target )
		{
            ...
            if ( ( target.Region is CustomRegion && ((CustomRegion)target.Region).Controller.AlwaysGrey ) || target.Region.IsPartOf( "Buccaneer's Den" ) )
                return Notoriety.CanBeAttacked;
			...
			if ( ft != null && tt != null && ft.Game == tt.Game )
...
*/
		...
		public static bool CheckAggressed( List<AggressorInfo> list, Mobile target )
		{
			for( int i = 0; i < list.Count; ++i )
			{
				...
				if( !info.CriminalAggression && !info.BeneficialAggression && info.Defender == target )
					return true;
			}
