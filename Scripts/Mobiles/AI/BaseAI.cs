				...
				if ( !m_Mobile.Deleted && m_Mobile.Controlled && m_From.CheckAlive() )
				{
					...
					switch( m_Order )
					{
						...
						case OrderType.Unfriend:
						{
							...
							else if ( m_Mobile.CheckControlChance( m_From ) )
...
*/
				if ( (SolenHelper.CheckRedFriendship( from ) && (target is IRedSolen)) || (SolenHelper.CheckBlackFriendship( from ) && (target is IBlackSolen)) )
				{
				...
				if ( e.Mobile.Alive && (isOwner || isFriend) )
				{
					...
					for( int i = 0; i < keywords.Length; ++i )
					{
						...
						switch( keyword )
						{
							...
							{
								if ( !isOwner || !m_Mobile.CheckControlChance( e.Mobile ))
									break;
		...
		{
			if ( m_Mobile.ControlMaster == null || m_Mobile.Deleted )
				return false;
		...
		public virtual bool DoOrderFollow()
		{
			...
			else if ( m_Mobile.ControlTarget != null && !m_Mobile.ControlTarget.Deleted && m_Mobile.ControlTarget != m_Mobile && (m_Mobile.ControlTarget.Alive || m_Mobile.ControlTarget == m_Mobile.ControlMaster) && (!m_Mobile.ControlTarget.Hidden || m_Mobile.ControlTarget == m_Mobile.ControlMaster) && m_Mobile.InRange( m_Mobile.ControlTarget, m_Mobile.RangePerception * 2 ) )
			{
		...
		public virtual bool DoOrderGuard()
		{
			...
			Mobile mastercombatant = controlMaster.Combatant;

			if ( mastercombatant != null && !mastercombatant.Deleted && mastercombatant.GetDistanceToSqrt( m_Mobile ) <= m_Mobile.RangePerception )
			{
				if ( combatant == null || mastercombatant.GetDistanceToSqrt( controlMaster ) < combatant.GetDistanceToSqrt( controlMaster ) )
				{
					combatant = mastercombatant;
					m_Mobile.DebugSay( "My master is defending himself! I will attack them too!" );
				}
			}
			else
			{
		...
		public virtual bool DoOrderAttack()
		{
			...
			if ( m_Mobile.ControlTarget == null || m_Mobile.ControlTarget.Deleted || m_Mobile.ControlTarget.Map != m_Mobile.Map || !m_Mobile.ControlTarget.Alive || m_Mobile.ControlTarget.IsDeadBondedPet || !m_Mobile.InRange( m_Mobile.ControlTarget, m_Mobile.RangePerception * 2 ) )
			{
		...
		public virtual bool AcquireFocusMob( int iRange, FightMode acqType, bool bPlayerOnly, bool bFacFriend, bool bFacFoe )
		{
			...
			if ( map != null )
			{
				...
				foreach( Mobile m in eable )
				{
					...
					//Monsters of the same champion spawn are buddies!
					if ( bc != null && bc.ChampSpawn != null && bc.ChampSpawn == m_Mobile.ChampSpawn )
						continue;
		...
		private void ReturnToHome()
		{
			...
			Spawner sp = m_Mobile.Spawner as Spawner;

			if ( sp != null )
			{
				Point3D loc = sp.GetSpawnPosition();

				if ( loc != Point3D.Zero )
				{
					m_Mobile.MoveToWorld( loc, sp.Map );
					return;
				}
			}
