...
namespace Server.Mobiles
{
	public class MageAI : BaseAI
	{
		...
		private const double HealChance = 0.0; // 0.5% chance to heal at gm magery
		private const double TeleportChance = 0.0; // 0.5% chance to teleport at gm magery
		private const double DispelChance = 0.50; // 50% chance to dispel at gm magery
//		private const double RevealChance = 0.005; // 1% chance to reveal at gm magery
...
/*
				...
				case  6: return new PoisonSpell( m_Mobile, null );
				case  7: return new CurseSpell( m_Mobile, null );
				case  8: return new LightningSpell( m_Mobile, null );

				case  9: return new ParalyzeSpell( m_Mobile, null );
				case 10: return new MindBlastSpell( m_Mobile, null );
				case 11: return new EnergyBoltSpell( m_Mobile, null );
				case 12: return new ExplosionSpell( m_Mobile, null );
				default: return new FlameStrikeSpell( m_Mobile, null );
*/
			}
		}
			...
			switch ( Utility.Random( 3 ) )
			{
				...
				case 2: // Set up a combo
				{
					if ( m_Mobile.Mana < 40 && m_Mobile.Mana > 15 )
					{
						...
						else if ( !( ( m_Mobile is IEvoCreature || m_Mobile is EvolutionDragon ) && c is PlayerMobile ) && !c.Poisoned )
						{
							spell = new ParalyzeSpell( m_Mobile, null );
						}
					}
					...
					{
						if ( !( ( m_Mobile is IEvoCreature || m_Mobile is EvolutionDragon ) && c is PlayerMobile ) && Utility.Random( 2 ) == 0 && !c.Paralyzed && !c.Frozen && !c.Poisoned )
						{
							m_Combo = 0;
							spell = new ParalyzeSpell( m_Mobile, null );
						}
		...
		public override bool DoActionCombat()
		{
			...
			if ( m_Mobile.Spell == null && DateTime.Now > m_NextCastTime && m_Mobile.InRange( c, Core.ML ? 10 : 12 ) )
			{
				...
				else if ( SmartAI && !( ( m_Mobile is IEvoCreature || m_Mobile is EvolutionDragon ) && c is PlayerMobile ) && (c.Spell is HealSpell || c.Spell is GreaterHealSpell) && !c.Poisoned ) // They have a heal spell out
				{
					spell = new PoisonSpell( m_Mobile, null );
				}
				else
