...
namespace Server.Mobiles
{
	...
	public class BaseCreature : Mobile, IHonorTarget
	{
		...
		public virtual TimeSpan BondingAbandonDelay{ get{ return TimeSpan.FromDays( 2.0 ); } }
		...
		public virtual bool UnprovokableTarget{ get{ return Core.AOS && ( BardImmune || m_IsDeadPet ); } }
		...
		public virtual void BreathDealDamage( Mobile target )
		{
			...
			{ // Unresistable damage even in AOS
				...
				AOS.Damage( target, this, damage, physDamage, fireDamage, coldDamage, poisDamage, nrgyDamage );
		}
		...
		*/
		...
		{
			if ( mob == null || !Faction.IsFactionFacet( mob.Map ) || FactionAllegiance == null )
		...
		public virtual bool IsEnemy( Mobile m )
		{
            ...
            if ( ourFaction != null && Faction.IsFactionFacet(Map) && !RegionIsNoFactions )
			{
...
//						value = 65000;
		...
		public virtual bool CanDestroyObstacles
		{
			get
			{
				...
				return Controlled && !(Region is StrongholdRegion); //Al: All tamed creatures can destroy obstacles
				//return false;
			}
		}
...
*/
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			if ( version <= 14 ) //Valor is no longer serialized
			{
				reader.ReadBool();
				reader.ReadInt();
			}
			...
			if ( ( deleteTime > TimeSpan.Zero || LastOwner != null ) && !Controlled/* && !IsStabled*/ )
			{
				if ( deleteTime == TimeSpan.Zero )
					deleteTime = TimeSpan.FromDays( 2.0 );
...
*/
		...
		public virtual void GenerateLoot( bool spawning )
		{
			...
			else if ( FavoriteFood != FoodType.None && Utility.Random( 4 ) == 0 )
			{
...
*/
        ...
        private void PackQuestRewards()
        {
            //Doom quest and idol system
            if
            (
                !NoKillAwards &&
                (
                //Doom summoning quest
                    Region.Name == "Doom" ||
                //Idol system
                    ...
                    Region.Name == "Deceit" ||
                    Region.Name == "Destard" ||
                    Region.Name == "Hythloth" ||
                    Region.Name == "Despise" ||
                    Region.Name == "Covetous" ||
                    Region.Name == "Wrong"

                )
            )
            {
                //Doom summoning quest /and/ the Idol System use the same calculation for bones/fragments
                int bones = Engines.Quests.Doom.TheSummoningQuest.GetDaemonBonesFor(this);
                if (bones > 0)
                {
                    switch (Region.Name)
                    {
                        ...
                        case "Shame":
                            PackItem(new FragmentCrystal(QuestType.Shame, bones));
                            break;
                        case "Deceit":
                            PackItem(new FragmentCrystal(QuestType.Deceit, bones));
                            break;
                        case "Destard":
                            PackItem(new FragmentCrystal(QuestType.Destard, bones));
                            break;
                        case "Hythloth":
                            PackItem(new FragmentCrystal(QuestType.Hythloth, bones));
                            break;
                        case "Despise":
                            PackItem(new FragmentCrystal(QuestType.Despise, bones));
                            break;
                        case "Covetous":
                            PackItem(new FragmentCrystal(QuestType.Covetous, bones));
                            break;
                        case "Wrong":
                            PackItem(new FragmentCrystal(QuestType.Wrong, bones));
                            break;
...
/*			if ( IsStabled ) //WTF?
			{
				Console.WriteLine( "*** Stabled Pet Deleted at {2} {0} {1}", Serial, GetType().ToString(), DateTime.Now );
				throw new Exception( String.Format( "*** Stabled Pet Deleted at {2} {0} {1}", Serial, GetType().ToString(), DateTime.Now ) );
			} */
...
/*
				Region reg = null;
				if ( se is SpawnEntry )
					reg = ((SpawnEntry)se).Region;
				else if ( se is Item )
					reg = Region.Find( ((Item)se).Location, ((Item)se).Map );

				if ( se != null && !se.UnlinkOnTaming && ( New == null || !New.AcceptsSpawnsFrom( reg ) ) )
				{
					this.Spawner.Remove( this );
					this.Spawner = null;
				}
