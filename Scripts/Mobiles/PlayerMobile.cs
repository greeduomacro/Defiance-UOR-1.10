...
namespace Server.Mobiles
{
	...
	public class PlayerMobile : Mobile, IHonorTarget, IComparable
	{
		...
		private void ValidateEquipment_Sandbox()
		{
			try
			{
				...
				for ( int i = items.Count - 1; i >= 0; --i )
				{
					...
					#region Ethics
					Ethics.EthicsItem ethicsItem = Ethics.EthicsItem.Find( item );
					if ( ethicsItem != null && ethicsItem.Ethic != ethic )
					{
...
//				return DeathMoveResult.MoveToBackpack;
		...
		public override void OnDeath( Container c )
		{
			...
			if ( Region is GameRegion || (Region is CustomRegion && ((CustomRegion)Region).Controller.DeletePlayerCorpse ))
				c.Delete();
			...
			if ( this.Kills >= Mobile.MurderCount && DateTime.Now >= m_NextJustAward )
			{
...
*/
	  ...
	  public TimeSpan NextHuntContract
	  {
		...
		public static readonly TimeSpan ShortTermDuration = TimeSpan.FromHours( 6.0 );
		public static readonly TimeSpan LongTermDuration = TimeSpan.FromHours( 12.0 );
		...
		public override void DoHarmful( Mobile target, bool indirect ) //Is this all types of aggression?
		{
			...
			{
				Ethics.Player sourceEPL = m_EthicPlayer;
				Ethics.Player targetEPL = ((PlayerMobile)target).EthicPlayer;

				Guild fromGuild = Guild as Guild;
				Guild targetGuild = target.Guild as Guild;

				if( fromGuild == null || targetGuild == null || (targetGuild != fromGuild && !fromGuild.IsAlly( targetGuild ) && !fromGuild.IsEnemy( targetGuild )) )
					if ( !Faction.IsFactionFacet( Map ) || Faction.Find( target, true ) == null || Faction.Find( this, true ) == null ) //No factions here, or one or other is not in factions
						if ( sourceEPL == null && targetEPL != null ) //Doing harmful to an evil/hero, while outside of the system
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			switch ( version )
			{
				...
				case 24:
				{
					if ( version <= 28 )
					{
						reader.ReadDouble(); //Valor Gain
						reader.ReadDeltaTime(); //Valor Time
					}
					else
				...
				case 23:
				{
					if ( version <= 28 )
					{
						reader.ReadDeltaTime(); //m_LastKill
						reader.ReadInt(); //m_Rank
						reader.ReadBool(); //m_ShowPvpTitle
						reader.ReadBool(); //m_IsResKillProtected
					}
					else
				...
				case 22:
				{
					if ( version <= 28 )
					{
						reader.ReadInt(); //m_TotalPointsLost
						reader.ReadInt(); //m_TotalPointsSpent
						reader.ReadTimeSpan(); //ResKillTime
						reader.ReadMobile(); //m_LastPwned
						reader.ReadInt(); //m_TotalResKills
						reader.ReadInt(); //m_TotalResKilled
						reader.ReadMobile(); //m_LastPwner
						reader.ReadInt(); //m_TotalPoints
						reader.ReadInt(); //m_TotalWins
						reader.ReadInt(); //m_TotalLoses
						reader.ReadString(); //m_PvpRank
					}
					else
				...
				case 21:
				{
					if ( version <= 28 )
					{
						reader.ReadInt(); //m_Points AGAIN?
						reader.ReadInt(); //minPoints AGAIN?
					}
					else
					{
				...
				case 19:
				{
					if ( version <= 28 )
					{
						reader.ReadInt(); //m_Points
						reader.ReadInt(); //minPoints
					}
					else
					{
...
//						Timer.DelayCall( TimeSpan.Zero, new TimerCallback( RevertHair ) );
		...
		public override void Serialize( GenericWriter writer )
		{
			...
			writer.Write( (DateTime) m_NextHuntContract );
		...
		public override void OnSingleClick( Mobile from )
		{
			...
			bool RegionIsNoFactions = (Region is CustomRegion && ((CustomRegion)Region).Controller.NoFactionEffects);
			if ( Faction.IsFactionFacet( Map ) && !RegionIsNoFactions )
			{
				PlayerState pl = PlayerState.Find( this );
...
*/
		...
		[CommandProperty(AccessLevel.Administrator)]
		public bool PageSpy{ get{ return m_PageSpy; } set { m_PageSpy = value; } }
