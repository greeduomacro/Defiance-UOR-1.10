...
namespace Server.Multis
{
	public abstract class BaseHouse : BaseMulti
	{
		...
		public static int MaxFriends { get { return !Core.AOS ? 150 : 140; } }
		public static int MaxBans { get { return !Core.AOS ? 150 : 140; } }
		...
		public virtual TimeSpan DonatorDecayPeriod{ get{ return TimeSpan.FromDays( 360.0 ); } }

		public virtual DecayType DecayType
		{
			get
			{
				if ( m_RestrictDecay || !DecayEnabled || DecayPeriod == TimeSpan.Zero || TestCenter.Enabled )
				...
				PlayerMobile owner = m_Owner as PlayerMobile;

				if ( owner != null && owner.DonationTimeLeft > TimeSpan.Zero && owner.DonationTimeLeft < TimeSpan.MaxValue )
					return DecayType.Ageless;
...
*/
		...
		public virtual DecayLevel DecayLevel
		{
			get
			{
				...
				//Non-Perma Subscribers will always be age-less until the end of their subscription.  If they do not login within 360 days, their house collapses.
				//Perma subscribers will not say age-less, and will decay in 360 days.
				if ( m_Owner is PlayerMobile && ((PlayerMobile)m_Owner).DonationTimeLeft == TimeSpan.MaxValue )
					decayperiod = DonatorDecayPeriod; //360 day decay period
		...
		public virtual bool CheckDecay()
		{
			...
			{
				TimeSpan donation = TimeSpan.Zero;
				PlayerMobile owner = m_Owner as PlayerMobile;
				if ( owner != null )
					donation = owner.DonationTimeLeft;

				Console.WriteLine( "House Decayed: Account {2} - LastRefresh {0} - Donation Time {1}", m_LastRefreshed.ToString(), donation.ToString(), ( m_Owner != null && m_Owner.Account != null ) ? m_Owner.Account.ToString() : "null" );

				if ( donation > TimeSpan.Zero ) // Subscriber
					Timer.DelayCall( TimeSpan.Zero, new TimerCallback( DecayDonator_Sandbox ) );
				else
		...
		public virtual void DecayDonator_Sandbox()
		{
			if ( Deleted )
				return;
			...
			HouseDecayingCrate crate = MoveAllDecayingToCrate();
			m_Owner.BankBox.DropItem( crate );

			if ( Price > 0 )
				m_Owner.BankBox.DropItem( new BankCheck( Price ) );
			Item deed = GetDeed();
			if ( deed != null )
				crate.DropItem( deed );
		...
		private List<VendorInventory> m_VendorInventories = new List<VendorInventory>();
		private List<RelocatedEntity> m_RelocatedEntities = new List<RelocatedEntity>();
				...
				if ( !addon.Deleted )
				{
					...
					if (addon is IAddon)
					{
						...
						if (addon is BaseAddon && ((BaseAddon)addon).RetainDeedHue)
						{
							...
							{
								AddonComponent c = baseAddon.Components[i];
		...
		public List<IEntity> GetHouseEntities()
		{
			...
			{
				if ( item is AllBlocker || item is LOSBlocker || item is Blocker )
					list.Add( item );
				else if ( item.Parent == null && item.Map != Map.Internal )
					list.Add( item );
			}
		...
		private class TransferItem : Item
		{
			...
			public override void OnSingleClick( Mobile from )
			{
				...
				LabelTo( from, 1153, "{0} owned by {1} located {2}", houseName, owner, location );
			}
		...
		public void Kick( Mobile from, Mobile targ )
		{
			...
			/*
			else if ( IsFriend( targ ) && Core.UOTD && !Core.ML )
			{
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			//We must add a timer because of multithreaded loading, which results in a null account, and therefore houses that collapse.
			Timer.DelayCall( TimeSpan.FromSeconds( 5.0 ), new TimerCallback( CheckDecay_OnLoad ) );
		}

		private void CheckDecay_OnLoad()
		{
			if ( !CheckDecay() )
			{
		...
		{
			List<Item> lockDowns = new List<Item>();
			...
			for ( int i = 0; i < lockDowns.Count; ++i )
				SetLockdown( lockDowns[i], true, true );
		}
		...
		public static void HandleDeletion( Mobile mob )
		{
			...
			for ( int i = 0; i < houses.Count; ++i )
			{
				...
				if ( trans == null && !canClaim && !house.RestrictDecay )
		...
		public List<Mobile> CoOwners{ get{ return m_CoOwners; } set{ m_CoOwners = value; } }
		public List<Mobile> Friends{ get{ return m_Friends; } set{ m_Friends = value; } }
		public List<Mobile> Access{ get{ return m_Access; } set{ m_Access = value; } }
		public List<Mobile> Bans{ get{ return m_Bans; } set{ m_Bans = value; } }
		public List<Item> Doors{ get{ return m_Doors; } set{ m_Doors = value; } }
		...
		public List<Item> Addons{ get{ return m_Addons; } set{ m_Addons = value; } }
		public List<Item> LockDowns{ get{ return m_LockDowns; } }
		public List<SecureInfo> Secures{ get{ return m_Secures; } }
		public HouseSign Sign{ get{ return m_Sign; } set{ m_Sign = value; } }
		public List<Mobile> PlayerVendors{ get{ return m_PlayerVendors; } }
		public List<Mobile> PlayerBarkeepers{ get{ return m_PlayerBarkeepers; } }
		public List<Item> VendorRentalContracts{ get{ return m_VendorRentalContracts; } }
		public List<VendorInventory> VendorInventories{ get{ return m_VendorInventories; } }
		public List<RelocatedEntity> RelocatedEntities{ get{ return m_RelocatedEntities; } }
		public MovingCrate MovingCrate{ get{ return m_MovingCrate; } set{ m_MovingCrate = value; } }
		public List<Mobile> InternalizedVendors{ get{ return m_InternalizedVendors; } }
	...
	#region Targets
	public class AddonTarget : Target
	{
		private bool m_Release;
		private BaseHouse m_House;

		public AddonTarget( bool release, BaseHouse house ) : base( 12, false, TargetFlags.None )
		{
			CheckLOS = false;
		...
		{
			if ( !from.Alive || m_House.Deleted || from.AccessLevel < AccessLevel.GameMaster )
				return;

			if ( targeted is Item )
			{
				Item item = targeted as Item;
				if ( item is AddonComponent ) //Get the actual addon itself
					item = ((AddonComponent)item).Addon;

				if ( item.RootParent != null )
					from.SendMessage( "You cannot add items within a container." );
				else if ( item.Movable )
					from.SendMessage( "You cannot add items which are movable." );
				else if ( BaseHouse.CheckLockedDownOrSecured( item ) )
					from.SendMessage( "You cannot add items which are locked or secured." );
				else
				{
					if ( m_Release )
					{
						if ( m_House.Addons.Contains( item ) )
						{
							m_House.Addons.Remove( item );
							from.SendMessage( "The item has been removed from the house." );
						}
						else
							from.SendMessage( "The item is not part of the house." );
					}
					else if ( m_House.Addons.Contains( item ) )
						from.SendMessage( "The item is already part of the house." );
					else
					{
						m_House.Addons.Add( item );
						from.SendMessage( "The item has been added to the house." );
					}
				}
			}
			else
				from.SendMessage( "You cannot add that to the house." );
		}
	}
	...
	public class SetSecureLevelEntry : ContextMenuEntry
	{
		...
		public static ISecurable GetSecurable( Mobile from, Item item )
		{
			...
			{
				bool isOwned = item is BaseDoor && house.Doors.Contains( (BaseDoor)item );
