...
namespace Server.Multis
{
	public class HouseSign : Item
	{
		...
		public Mobile OriginalOwner{ get{ return m_OrgOwner; } set{ m_OrgOwner = value; } }
		...
		public override void OnSingleClick( Mobile from )
		{
			if ( m_Owner != null && BaseHouse.DecayEnabled && m_Owner.DecayPeriod != TimeSpan.Zero )
			{
				...
				switch ( m_Owner.DecayLevel )
				{
					...
					case DecayLevel.DemolitionPending: message = "awaiting demolition"; break;
					default:					message = "in danger of collapsing"; break;
				}

				LabelTo( from, "This house is {0}. {1}", message, ( m_Owner.Owner is PlayerMobile && ((PlayerMobile)m_Owner.Owner).DonationTimeLeft > TimeSpan.Zero ) ? "(defiance blessed)" : String.Empty );
			}
