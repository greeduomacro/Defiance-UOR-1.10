...
namespace Server.Regions
{
	public class HouseRegion : BaseRegion
	{
		...
		public override void OnSpeech( SpeechEventArgs e )
		{
			...
			else if ( speech.IndexOf( "i wish to add this" ) >= 0 && from.AccessLevel >= AccessLevel.GameMaster )
			{
				from.SendMessage( "Choose the item you wish to add to the house." );
				from.Target = new AddonTarget( false, m_House );
			}
			else if ( speech.IndexOf( "i wish to remove this" ) >= 0 && from.AccessLevel >= AccessLevel.GameMaster )
			{
