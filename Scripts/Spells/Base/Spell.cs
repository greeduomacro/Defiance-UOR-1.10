...
*/
		...
		public virtual void OnCasterHurt()
		{
			...
			if ( IsCasting )
			{
				...
				//else if ( MagicReflectSpell.Table[m_Caster] != null )
				//	disturb = false;
		...
		{
			if ((m_Caster.Region is CustomRegion) && ((CustomRegion)m_Caster.Region).Controller.CastWithoutReagents)
				return true;

			if ( m_Scroll != null || !m_Caster.Player || m_Caster.AccessLevel > AccessLevel.Counselor )
				return true;
		...
		*/
		...
		public virtual bool CheckSequence()
		{
			...
			if ( m_Caster.Deleted || !m_Caster.Alive || m_Caster.Spell != this || m_State != SpellState.Sequencing )
				DoFizzle();
			else if ( m_Scroll != null && !(m_Scroll is Runebook) && ( m_Scroll.Amount <= 0 || m_Scroll.Deleted || m_Scroll.RootParent != m_Caster || (m_Scroll is BaseWand && (((BaseWand)m_Scroll).Charges <= 0 || m_Scroll.Parent != m_Caster)) || (m_Scroll is GnarledStaff && (((GnarledStaff)m_Scroll).Charges <= 0 || m_Scroll.Parent != m_Caster)) ) )
				DoFizzle();
