...
namespace Server.Spells
{
	...
	public class SpellHelper
	{
		...
		public static TimeSpan[] NewDamageDelays = new TimeSpan[8]
			{
		...
		public static TimeSpan GetDamageDelayForSpell( Spell sp )
		{
			...
			if ( sp is MagerySpell )
				return NewDamageDelays[(int)((MagerySpell)sp).Circle];
		...
		public static bool CheckFactionAggressed(Mobile m)
		{
			PlayerMobile pm = m as PlayerMobile;
			if (pm == null || Faction.Find(m, true, true)==null) return false;
			Faction mFaction = Faction.Find(m, true, true);
			...
			for (int i = 0; i < m.Aggressors.Count; ++i)
			{
				...
				if (
						...
						Faction.Find(info.Attacker, true, true) != null &&
						Faction.Find(info.Attacker, true, true) != mFaction
					) return true;
			}
			return false;
		}
		...
		public static void GetSurfaceTop( ref IPoint3D p )
		{
			...
			else if( p is StaticTarget )
			{
				...
				if( (t.Flags & TileFlag.Surface) == 0 ) //Is not a surface tile/static
		...
		public static bool ValidIndirectTarget( Mobile from, Mobile to )
		{
			...
			if( p != null && p.Contains( to ) && !( to.Region is GameRegion && ((GameRegion)to.Region).AllowHarmful( from, to ) ) )
				return false;

			BaseCreature tbc = to as BaseCreature;
			BaseCreature fbc = from as BaseCreature;
			...
			if ( tbc != null && fbc != null && !tbc.Controlled && !fbc.Controlled && !tbc.BardProvoked && !fbc.BardProvoked && tbc.Team == fbc.Team )
				return false;
			...
			return (noto != Notoriety.Innocent || from.Kills >= 0);
		}
...
/* Recall From */		{ true,			true,			true,			false,		true,			true,			false,		false,				false,				false,				true,			true,			true,				false,				false },
/* Recall To */			{ true,			true,			false,			false,		true,			false,			false,		false,				false,				false,				false,			false,			false,				false,				false },
/* Gate From */			{ true,			false,			false,			false,		true,			false,			false,		false,				false,				false,				false,			false,			false,				false,				false },
/* Gate To */			{ true,			false,			false,			false,		true,			false,			false,		false,				false,				false,				false,			false,			false,				false,				false },
/* Mark In */			{ true,			false,			false,			false,		true,			false,			false,		false,				false,				false,				false,			false,			false,				false,				false },
...
/* Tele To */			{ true,			true,			true,			true,		true,			true,			true,		false,				true,				false,				false, 			true,			true,				true,				true },
		...
		public static bool CheckTravel( Mobile caster, Map map, Point3D loc, TravelCheckType type )
		{
			...
			if( caster != null && caster.AccessLevel == AccessLevel.Player )
			{
				...
				if ( ga != null && ga.IsPartOf( typeof( Regions.GreenAcres ) ) )
				{
					caster.SendLocalizedMessage( 1019004 ); // You are not allowed to travel there.
					return false;
				}
