		...
		public override string Compile( NetState ns )
		{
			...
			if ( entry != null )
			{
				...
				else
				{
					...
					{
						case GumpHtmlLocalizedType.Plain:
							return String.Format( "{{ htmlgump {0} {1} {2} {3} {4} {5} {6} }}", m_X, m_Y, m_Width, m_Height, Parent.Intern( entry.Text ), m_Background ? 1 : 0, m_Scrollbar ? 1 : 0 );

						case GumpHtmlLocalizedType.Color:
							return String.Format( "{{ htmlgump {0} {1} {2} {3} {4} {5} {6} }}", m_X, m_Y, m_Width, m_Height, Parent.Intern( String.Format( "<BASEFONT COLOR=#{1:X6}>{0}</BASEFONT>", entry.Text, m_Color ) ), m_Background ? 1 : 0, m_Scrollbar ? 1 : 0 );

						default: // GumpHtmlLocalizedType.Args
							return String.Format( "{{ htmlgump {0} {1} {2} {3} {4} {5} {6} }}", m_X, m_Y, m_Width, m_Height, Parent.Intern( String.Format( "<BASEFONT COLOR=#{1:X6}>{0}</BASEFONT>", entry.CombinedWithArguments( m_Args ), m_Color ) ), m_Background ? 1 : 0, m_Scrollbar ? 1 : 0 );
					}
				}
			}
			else
