...
namespace Server
{
	...
	{
		int DyedHue{ get; set; }
		TextDefinition FailMessage{ get; }
		TextDefinition TargetMessage{ get; }
	}
...
*/
		...
		public static void Serialize( BounceInfo info, GenericWriter writer )
		{
			...
			else
			{
				...
				writer.WriteEntity( info.m_Parent as IEntity );
/*
...
*/
	...
	public class Item : IEntity, IHued, IPoint3D, IComparable<Item>, ISerializable
	{
		...
		public bool Dyable{ get{ return GetFlag( ImplFlag.Dyable ); } set{ SetFlag( ImplFlag.Dyable, value ); InvalidateProperties(); } }

		public virtual bool DisplayDyable{ get{ return true; } }
		public static Type DefaultDyeType = null;
		public virtual Type DyeType{ get{ return DefaultDyeType; } }
...
/*
			if ( GetFlag( ImplFlag.Dyable ) && DisplayDyable )
				list.Add( 1041522, "Dyable\t\t" ); //~1~~2~~3~
*/
		...
		/// <summary>
		/// Overridable. Method checked to see if this item may be equiped while casting a spell. By default, this returns false. It is overridden on spellbook and spell channeling weapons or shields.
		/// </summary>
		...
		public void LabelTo( Mobile to, int hue, int number )
		{
			...
			if ( entry != null )
			{
				if ( entry.State == LState.Original ) //Client Localized
					...
					to.Send( new UnicodeMessage( m_Serial, m_ItemID, MessageType.Label, hue, 3, to.Language, "", entry.Text ) );
			}
		}
		...
		public void LabelTo( Mobile to, int hue, int number, string args )
		{
			...
			if ( entry != null )
			{
				if ( entry.State == LState.Original ) //Client Localized
					...
					to.Send( new UnicodeMessage( m_Serial, m_ItemID, MessageType.Label, hue, 3, to.Language, "", entry.CombinedWithArguments( args ) ) );
			}
		}
		...
		{
			to.Send( new UnicodeMessage( m_Serial, m_ItemID, MessageType.Label, hue, 3, to.Language, "", text ) );
		}

		public void LabelTo( Mobile to, int hue, string format, params object[] args )
		{
			LabelTo( to, String.Format( format, args ), hue );
		}

		#region Extended LabelTo

		public void ExtendedLabelTo( Mobile to, int number )
		{
			ExtendedLabelTo( to, number, 0x3B2 );
		}

		public void ExtendedLabelTo( Mobile to, int number, params object[] args )
		{
			ExtendedLabelTo( to, 0x3B2, number, args );
		}

		public void ExtendedLabelTo( Mobile to, int number, int hue )
		{
			StringEntry entry = to.LTable[number];
			if ( entry != null )
				ExtendedLabelTo( to, hue, entry.Text );
		}

		public void ExtendedLabelTo( Mobile to, int hue, int number, params object[] args )
		{
			StringEntry entry = to.LTable[number];
			if ( entry != null )
				ExtendedLabelTo( to, hue, String.Format( entry.Text, args ) );
		}

		internal void ExtendedLabelTo( Mobile to, int hue, string text )
		{
			to.Send( new UnicodeMessage( m_Serial, m_ItemID, MessageType.Label, hue, 3, to.Language, "", text ) );
		}
		...
		public void LabelToAffix( Mobile to, int hue, int number, AffixType type, string affix, string args )
		{
			...
			if ( entry != null )
			{
				...
				{
					bool append = ( type & AffixType.Append ) != 0;
					to.Send( new UnicodeMessage( m_Serial, m_ItemID, MessageType.Label, hue, 3, to.Language, "", append ? entry.AppendArguments( args ) : entry.PrependArguments( args ) ) );
				}
			}
		}
/*
		public virtual string LabelAttributesTo( Mobile to )
		{
			...
			if ( DisplayLootType )
			{
				...
				if ( builder.Length > 0 && !String.IsNullOrEmpty( loot ) )
					builder.Append( "/" );

				builder.Append( loot );
			}

			if ( DisplayDyable && Dyable )
			{
				if ( builder.Length > 0 )
					builder.Append( "/" );

				builder.Append( "Dyable" );
			}
...
*/
		...
		{
			if ( m_LootType == LootType.Blessed )
				LabelTo( to, 1153, 1041362 ); // (blessed)
			else if ( m_LootType == LootType.Cursed )
				LabelTo( to, "(cursed)", 38 );
		}
/*
		public virtual void LabelDyableTo( Mobile to )
		{
			if ( Dyable )
				LabelTo( to, "(dyable)", 1346 );
		}
*/
		...
		{
			return ( Decays && m_Spawner == null && Parent == null && Map != Map.Internal && Region.Find( Location, Map ).OnDecay( this ) );
		}
		...
		{
			return dropped.Stackable && Stackable && dropped.GetType() == GetType() && dropped.ItemID == ItemID && dropped.Hue == Hue && dropped.Name == Name && (dropped.Amount + Amount) <= 60000;
		}
		...
		public virtual void Serialize( GenericWriter writer )
		{
			...
			ImplFlag implFlags = ( m_Flags & ( ImplFlag.Visible | ImplFlag.Movable | ImplFlag.Stackable | ImplFlag.Insured | ImplFlag.PaidInsurance | ImplFlag.QuestItem | ImplFlag.Dyable ) );
			...
			{
				writer.WriteEntity( m_Parent as IEntity );
/*
...
*/
						...
						if ( entity is Entity && (entity.Serial.IsMobile || entity.Serial.IsItem) ) //Its not a World Entity
							Delete();
						else
							m_Parent = entity;
					}
...
*/
					...
					if ( m.CanSee( this ) && m.InRange( worldLoc, GetUpdateRange( m ) ) )
					{
						if ( p == null )
						{
							if ( ascii )
								...
								p = new UnicodeMessage( m_Serial, m_ItemID, type, hue, 3, m.Language, this.Name, text );
		...
		public void PublicOverheadMessage( MessageType type, int hue, int number, string args )
		{
			if ( m_Map != null )
			{
				...
				foreach ( NetState state in eable )
				{
					...
					if ( m.CanSee( this ) && m.InRange( worldLoc, GetUpdateRange( m ) ) )
					{
						StringEntry entry = m.LTable[number];
						if ( entry != null )
						{
							if ( entry.State == LState.Original ) //Client Localized
							{
							...
							{
								Packet newp = Packet.Acquire( new UnicodeMessage( m_Serial, m_ItemID, type, hue, 3, m.Language, Name, entry.CombinedWithArguments( args ) ) );
								state.Send( newp );
								Packet.Release( newp );
							}
						}
		...
		{
			get { return m_Parent as Item; }
		}

		[CommandProperty( AccessLevel.GameMaster )]
		public Mobile ParentMobile
		{
			get { return m_Parent as Mobile; }
		}
		...
		public void SendLocalizedMessageTo( Mobile to, int number )
		{
			...
			{
				if ( entry.State == LState.Original ) //Client Localized
					to.Send( new MessageLocalized( m_Serial, m_ItemID, MessageType.Regular, 0x3B2, 3, number, "", "" ) );
				else
					to.Send( new UnicodeMessage( m_Serial, m_ItemID, MessageType.Regular, 0x3B2, 3, to.Language, "", entry.Text ) );
			}
		}
		...
		public void SendLocalizedMessageTo( Mobile to, int number, string args )
		{
			...
			{
				if ( entry.State == LState.Original ) //Client Localized
					to.Send( new MessageLocalized( m_Serial, m_ItemID, MessageType.Regular, 0x3B2, 3, number, "", args ) );
				else
					to.Send( new UnicodeMessage( m_Serial, m_ItemID, MessageType.Regular, 0x3B2, 3, to.Language, "", entry.CombinedWithArguments( args ) ) );
			}
		}
		...
		public void SendLocalizedMessageTo( Mobile to, int number, AffixType affixType, string affix, string args )
		{
			...
			if ( entry != null )
			{
				...
				else
				{
					...
					to.Send( new UnicodeMessage( m_Serial, m_ItemID, MessageType.Regular, 0x3B2, 3, to.Language, "", append ? entry.AppendArguments( args ) : entry.PrependArguments( args ) ) );
				}
			}
		}
		...
		public virtual void OnSingleClick( Mobile from )
		{
			...
			//string attributes = LabelAttributesTo( from );

			//if ( !String.IsNullOrEmpty( attributes ) )
			//	LabelTo( from, String.Format( "[{0}]", attributes ), 1153 );
			...
			if ( ns != null )
			{
				...
				{
					ns.Send( new UnicodeMessage( m_Serial, m_ItemID, MessageType.Label, 0x3B2, 3, from.Language, "", this.Name + ( m_Amount > 1 ? " : " + m_Amount : "" ) ) );
				}
			}
		}
