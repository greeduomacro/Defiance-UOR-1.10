 ...
 *
 *   $Id: Mobile.cs 551 2010-10-15 03:54:16Z mark $
 *
 ***************************************************************************/
...
*/
	...
	public class Mobile : IEntity, IHued, IComparable<Mobile>, ISerializable
	{
		...
		private Race m_Race;
		private int m_BloodHue;
		private string m_PreferredLanguage;
		#endregion

		public StringList LTable{ get{ if ( m_NetState != null ) return m_NetState.LTable; else return null; } set{ if ( m_NetState != null ) m_NetState.LTable = value; } }
		...
		public string PreferredLanguage
		{
			...
			set
			{
				...
				if ( m_NetState != null )
				{
					m_NetState.LTable = StringList.GetList( m_PreferredLanguage );
					if ( m_NetState.LTable == null )
						m_NetState.LTable = StringList.Default;
				}
			}
		}
		...
		public bool Running{ get{ return ( Direction & Direction.Running ) != 0; } }
		...
		private static int m_MurderCount = 5;

		public static int MaxPlayerResistance { get { return m_MaxPlayerResistance; } set { m_MaxPlayerResistance = value; } }
		public static int MurderCount{ get{ return m_MurderCount; } set{ m_MurderCount = value; } }
		...
		public void AggressiveAction( Mobile aggressor )
		{
		...
		{
			AggressiveAction( aggressor, criminal, false );
		}

		public virtual void AggressiveAction( Mobile aggressor, bool criminal, bool healing )
		{
			AggressiveAction( aggressor, criminal, healing, true );
		}

		public virtual void AggressiveAction( Mobile aggressor, bool criminal, bool healing, bool sendMessage )
		{
			if( aggressor == this )
				return;

			AggressiveActionEventArgs args = AggressiveActionEventArgs.Create( this, aggressor, criminal, healing, sendMessage );
			...
			for( int i = 0; i < list.Count; ++i )
			{
				...
				if( info.Attacker == aggressor )
				{
					...
					info.BeneficialAggression = healing;
			...
			if( addAggressor )
			{
				m_Aggressors.Add( AggressorInfo.Create( aggressor, this, criminal, healing ) ); // new AggressorInfo( aggressor, this, criminal, true ) );
			...
			if( addAggressed )
			{
				aggressor.m_Aggressed.Add( AggressorInfo.Create( aggressor, this, criminal, healing ) ); // new AggressorInfo( aggressor, this, criminal, false ) );
		...
		private DateTime m_MoveRecordEnd;

		[CommandProperty( AccessLevel.Seer )]
		public DateTime EndQueue{ get{ return m_EndQueue; } set{ m_EndQueue = value; } }

		[CommandProperty( AccessLevel.Seer )]
		public DateTime MoveRecordEnd{ get{ return m_MoveRecordEnd; } set{ m_MoveRecordEnd = value; } }

		[CommandProperty( AccessLevel.Seer )]
		public int MoveRecordCount{ get{ return m_MoveRecords.Count; } }

		public Queue<MovementRecord> MoveRecords{ get{ return m_MoveRecords; } }
		...
		private static int m_FwdMaxSteps = 5;
		private static TimeSpan m_MoveRecordDelay = TimeSpan.FromSeconds( 5.0 );
		...
		public static TimeSpan MoveRecordDelay { get { return m_MoveRecordDelay; } set { m_MoveRecordDelay = value; } }

		public void ClearFastwalkStack()
		{
			ClearFastwalkStack( false );
		}

		public virtual void ClearFastwalkStack( bool delayed )
		{
			if ( !delayed || m_MoveRecordEnd > DateTime.Now )
			{
				if ( m_MoveRecords != null && m_MoveRecords.Count > 0 )
				{
					m_MoveRecords.Clear();
					m_MoveRecordEnd = DateTime.Now + m_MoveRecordDelay;
				}
				...
				if( from.CheckAlive() )
				{
					...
					else
					{
						...
						else
						{
							...
							//What if they are guard whacked?
							if ( from.Alive )
							{
								item.Internalize();
								from.Holding = item;
		...
		public void Heal( int amount, Mobile from, bool message )
		{
			...
			{
				StringEntry entry = m_NetState.LTable[1008158];
				if ( entry != null )
				{
					if ( entry.State == LState.Original ) //Client Localized
						...
						m_NetState.Send( new UnicodeMessage( Serial.MinusOne, -1, MessageType.Label, 0x3B2, 3, this.Language, "", entry.AppendArguments( amount.ToString() ) ) );
				}
			}
		}
		...
		public virtual void Deserialize( GenericReader reader )
		{
			...
			if ( version >= 32 )
				flags = (SaveFlag)reader.ReadEncodedInt();

			switch( version )
			{
				case 32:
				{
					if( GetSaveFlag( flags, SaveFlag.BloodHue ) )
						m_BloodHue = reader.ReadInt();
					else
						m_BloodHue = DefaultBloodHue;
				...
				case 30:
				{
					...
					if( GetSaveFlag( flags, SaveFlag.Hair ) )
						m_Hair = new HairInfo( reader );
					if( GetSaveFlag( flags, SaveFlag.FacialHair ) )
		...
		public virtual void Serialize( GenericWriter writer )
		{
			...
			SetSaveFlag( ref flags, SaveFlag.Hair,			m_Hair != null );
			SetSaveFlag( ref flags, SaveFlag.FacialHair,		m_FacialHair != null );
			SetSaveFlag( ref flags, SaveFlag.BloodHue,		m_BloodHue != DefaultBloodHue );
		...
		public int Fame
		{
			get
			{
				return m_AccessLevel > AccessLevel.Player ? 0 : m_Fame;
			}
			set
			{
				int oldValue = m_Fame;
		...
		public int Karma
		{
			get
			{
				return m_AccessLevel > AccessLevel.Player ? 0 : m_Karma;
			}
			set
			{
				int old = m_Karma;
		...
		[CommandProperty( AccessLevel.GameMaster )]
		public virtual int BloodHue{ get{ return m_BloodHue; } set{ m_BloodHue = value; } }
...
//			m_LTable = StringList.Default;
		...
		public int Kills
		{
			get
			{
				return m_AccessLevel > AccessLevel.Player ? 0 : m_Kills;
			}
			set
			{
				...
				if( m_Kills != value )
				{
					...
					if( (oldValue >= m_MurderCount) != (m_Kills >= m_MurderCount) )
					{
						Delta( MobileDelta.Noto );
						InvalidateProperties();
					}
		...
		public int ShortTermMurders
		{
			get
			{
				return m_AccessLevel > AccessLevel.Player ? 0 : m_ShortTermMurders;
			}
			set
			{
		...
		#region Extended overhead message

		public void PublicExtendedOverheadMessage( MessageType type, int hue, bool ascii, int number )
		{
			PublicExtendedOverheadMessage( type, hue, ascii, number, true );
		}

		public void PublicExtendedOverheadMessage( MessageType type, int hue, bool ascii, int number, bool noLineOfSight )
		{
			if( m_Map != null )
			{
				IPooledEnumerable eable = m_Map.GetClientsInRange( m_Location );
				...
				foreach( NetState state in eable )
				{
					...
					{
						StringEntry entry = state.Mobile.LTable[number];
						if ( entry != null )
						{
							Packet p = null;

							if( ascii )
								p = new AsciiMessage( m_Serial, Body, type, hue, 3, Name, entry.Text );
							else
								p = new UnicodeMessage( m_Serial, Body, type, hue, 3, m_Language, Name, entry.Text );
		...
		public void PrivateExtendedOverheadMessage( MessageType type, int hue, bool ascii, int number, NetState state )
		{
			if( state != null )
			{
				StringEntry entry = state.Mobile.LTable[number];
				...
				{
					if( ascii )
						state.Send( new AsciiMessage( m_Serial, Body, type, hue, 3, Name, entry.Text ) );
					else
						state.Send( new UnicodeMessage( m_Serial, Body, type, hue, 3, m_Language, Name, entry.Text ) );
				}
			}
		}

		public void LocalExtendedOverheadMessage( MessageType type, int hue, bool ascii, int number )
		{
			if( m_NetState != null )
			{
				StringEntry entry = m_NetState.LTable[number];
				if ( entry != null )
				{
					if( ascii )
						m_NetState.Send( new AsciiMessage( m_Serial, Body, type, hue, 3, Name, entry.Text ) );
					else
						m_NetState.Send( new UnicodeMessage( m_Serial, Body, type, hue, 3, m_Language, Name, entry.Text ) );
				}
			}
		}

		public void NonlocalExtendedOverheadMessage( MessageType type, int hue, bool ascii, int number )
		{
			if( m_Map != null )
			{
				Packet p = null;
					...
					if( state != m_NetState && state.Mobile.CanSee( this ) )
					{
						...
						if ( entry != null )
						{
							if( ascii )
								...
								p = new UnicodeMessage( m_Serial, Body, type, hue, 3, Language, Name, entry.Text );
				...
				foreach( NetState state in eable )
				{
					if( state.Mobile.CanSee( this ) && (noLineOfSight || state.Mobile.InLOS( this )) )
					{
						...
						if ( entry != null )
						{
							...
							{
								Packet newp = Packet.Acquire( new UnicodeMessage( m_Serial, Body, type, hue, 3, state.Mobile.Language, Name, entry.CombinedWithArguments( args ) ) );
								state.Send( newp );
								Packet.Release( newp );
							}
						}
		...
		public void PublicOverheadMessage( MessageType type, int hue, int number, AffixType affixType, string affix, string args, bool noLineOfSight )
		{
			if( m_Map != null )
			{
				bool append = ( affixType & AffixType.Append ) != 0;
				Packet p = null;
				...
				foreach( NetState state in eable )
				{
					if( state.Mobile.CanSee( this ) && (noLineOfSight || state.Mobile.InLOS( this )) )
					{
						...
						if ( entry != null )
						{
							...
							{
								Packet newp = Packet.Acquire( new UnicodeMessage( m_Serial, Body, type, hue, 3, state.Mobile.Language, Name, append ? entry.AppendArguments( args ) : entry.PrependArguments( args ) ) );
								state.Send( newp );
								Packet.Release( newp );
							}
						}
		...
		public void LocalOverheadMessage( MessageType type, int hue, int number, string args )
		{
			if( m_NetState != null )
			{
				...
				{
					if ( entry.State == LState.Original ) //Client Localized
						m_NetState.Send( new MessageLocalized( m_Serial, Body, type, hue, 3, number, Name, args ) );
					else
						m_NetState.Send( new UnicodeMessage( m_Serial, Body, type, hue, 3, m_Language, Name, entry.CombinedWithArguments( args ) ) );
				}
			}
		}
		...
		//These are used for clilocs, 500,000 -> 3,999,999
			...
			if( m_NetState != null )
			{
				...
				if ( entry != null )
				{
					if ( entry.State == LState.Original ) //Client Localized
						...
						m_NetState.Send( new UnicodeMessage( Serial.MinusOne, -1, MessageType.Regular, 0x3B2, 3, m_Language, "System", entry.Text ) );
				}
			}
		}
			...
			if( m_NetState != null )
			{
				...
				if ( entry != null )
				{
					if ( entry.State == LState.Original ) //Client Localized
					{
						...
						m_NetState.Send( new UnicodeMessage( Serial.MinusOne, -1, MessageType.Regular, hue, 3, m_Language, "System", entry.CombinedWithArguments( args ) ) );
				}
			}
		}
		...
		{
			SendLocalizedMessage( number, append, affix, String.Empty );
		}
		...
		public void SendLocalizedMessage( int number, bool append, string affix, string args, int hue )
		{
			if( m_NetState != null )
			{
				...
				{
					if ( entry.State == LState.Original ) //Client Localized
						m_NetState.Send( new MessageLocalizedAffix( Serial.MinusOne, -1, MessageType.Regular, hue, 3, number, "System", (append ? AffixType.Append : AffixType.Prepend) | AffixType.System, affix, args ) );
					else
						m_NetState.Send( new UnicodeMessage( Serial.MinusOne, -1, MessageType.Regular, hue, 3, m_Language, "System", append ? entry.AppendArguments( args ) : entry.PrependArguments( args ) ) );
				}
			}
		}
		...
		#region Extended SendMessage
/*

*/
		public void SendExtendedMessage( int number )
		{
			SendExtendedMessage( 0x3B2, number );
		}

		public void SendExtendedMessage( int number, params object[] args )
		{
			SendExtendedMessage( 0x3B2, number, args );
		}

		public void SendExtendedMessage( int hue, int number )
		{
			StringEntry entry = m_NetState.LTable[number];
			if ( entry != null )
				SendExtendedMessage( hue, entry.Text );
		}

		public void SendExtendedMessage( int hue, int number, params object[] args )
		{
			StringEntry entry = m_NetState.LTable[number];
			if ( entry != null )
				SendExtendedMessage( hue, String.Format( entry.Text, args ) );
		}

		//Internal method, post language deciphering.
		internal void SendExtendedMessage( int hue, string text )
		{
			if( m_NetState != null )
				m_NetState.Send( new UnicodeMessage( Serial.MinusOne, -1, MessageType.Regular, hue, 3, m_Language, "System", text ) );
		}

		public void SendExtendedAsciiMessage( int number )
		{
			SendExtendedAsciiMessage( 0x3B2, number );
		}

		public void SendExtendedAsciiMessage( int number, params object[] args )
		{
			SendExtendedAsciiMessage( 0x3B2, number, args );
		}

		public void SendExtendedAsciiMessage( int hue, int number )
		{
			StringEntry entry = m_NetState.LTable[number];
			if ( entry != null )
				SendExtendedAsciiMessage( hue, entry.Text );
		}

		public void SendExtendedAsciiMessage( int hue, int number, params object[] args )
		{
			StringEntry entry = m_NetState.LTable[number];
			if ( entry != null )
				SendExtendedAsciiMessage( hue, String.Format( entry.Text, args ) );
		}

		//Internal method, post language deciphering.
		internal void SendExtendedAsciiMessage( int hue, string text )
		{
			if( m_NetState != null )
		...
		{
			SendMessage( hue, 3, text );
		}

		public void SendMessage( int hue, int font, string text )
		{
			if( m_NetState != null )
				m_NetState.Send( new UnicodeMessage( Serial.MinusOne, -1, MessageType.Regular, hue, font, m_Language, "System", text ) );
		}
		...
		{
			SendAsciiMessage( hue, 3, text );
		}

		public void SendAsciiMessage( int hue, int font, string text )
		{
			if( m_NetState != null )
				m_NetState.Send( new AsciiMessage( Serial.MinusOne, -1, MessageType.Regular, hue, font, "System", text ) );
		}
