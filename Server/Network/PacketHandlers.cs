...
namespace Server.Network
{
	...
	public class PacketHandlers
	{
		...
		static PacketHandlers()
		{
			...
			Register( 0x96,   0,  true, new OnPacketReceive( GameCentralMonitor ) );
		...
		public static void EncodedCommand( NetState state, PacketReader pvSrc )
		{
			...
			if ( ph != null )
			{
				...
				{
					Console.WriteLine( "Client: {0}: Sent ingame packet (0xD7x{1:X2}) for a mobile which has been deleted" , state, packetID );
		...
		public static void TextCommand( NetState state, PacketReader pvSrc )
		{
			...
			switch ( type )
			{
				...
				case 0x2F:
				{
					...
					Item item = World.FindItem( Utility.ToInt32( split[0] ) );
					//Point3D loc = new Point3D( Utility.ToInt32( split[1] ), Utility.ToInt32( split[2] ), Utility.ToInt32( split[3] ) );

					if ( item != null && item.RootParent == m )
						m.SendMessage( "To inscribe this scroll, double click a scribe's pen." );
					else
						m.SendLocalizedMessage( 500446 ); // That is too far away.
//					EventSink.InvokeInscribeMenuRequest( new InvokeInscribeMenuRequest( m, World.FindItem( serial ), loc ) );
					break;
				}
				case 0x58: // Open door
				{
