...
namespace Server
{
	...
	public class Region : IComparable
	{
		...
		public List<Mobile> GetPlayers( bool nostaff )
		{
			...
			if ( m_Sectors != null )
			{
				for ( int i = 0; i < m_Sectors.Length; i++ )
				{
					...
					foreach ( Mobile player in sector.Players )
						if ( player.Region.IsPartOf( this ) && ( nostaff || player.AccessLevel == AccessLevel.Player ) )
							list.Add( player );
				}
			}
		...
		public int GetPlayerCount( bool nostaff )
		{
			int count = 0;
		...
		public static bool ReadEnum<T>( XmlElement xml, string attribute, ref T value, bool mandatory ) where T : struct
		{
			...
			{
				Console.WriteLine( "Could not parse {0} enum attribute '{1}' in element '{2}'", typeof( T ).GetType(), attribute, xml.Name );
				return false;
			}
#else
			Type type = typeof( T );
