...
namespace Server {
	public static class World {
		...
		private static Dictionary<Serial, BaseInstanceMap> m_InstanceMaps;
		...
		private static Queue<BaseInstanceMap> _addMapQueue, _deleteMapQueue;
		...
		public readonly static string MapIndexPath = Path.Combine( "Saves/Maps/", "Maps.idx" );
		public readonly static string MapTypesPath = Path.Combine( "Saves/Maps/", "Maps.tdb" );
		public readonly static string MapDataPath = Path.Combine( "Saves/Maps/", "Maps.bin" );
		...
		public static Dictionary<Serial, BaseInstanceMap> Maps {
			get { return m_InstanceMaps; }
		}
		...
		public static bool OnDelete( BaseInstanceMap map ) {
			if ( m_Saving || m_Loading ) {
				if ( m_Saving ) {
					AppendSafetyLog( "delete", map );
				}

				_deleteMapQueue.Enqueue( map );
		...
		{
			Broadcast( hue, 3, ascii, text );
		}

		public static void Broadcast( int hue, int font, bool ascii, string text )
		{
			Broadcast( AccessLevel.Player, hue, font, ascii, text );
		}

		public static void Broadcast( AccessLevel level, int hue, bool ascii, string text )
		{
			Broadcast( level, hue, 3, ascii, text );
		}

		public static void Broadcast( AccessLevel level, int hue, int font, bool ascii, string text )
		{
			Packet p;

			if ( ascii )
				p = new AsciiMessage( Serial.MinusOne, -1, MessageType.Regular, hue, font, "System", text );
			else
				p = new UnicodeMessage( Serial.MinusOne, -1, MessageType.Regular, hue, font, "ENU", "System", text );
		...
		public static void Broadcast( AccessLevel level, int hue, int font, bool ascii, string format, params object[] args )
		{
			Broadcast( level, hue, font, ascii, String.Format( format, args ) );
		}
		...
		public static void Broadcast( int hue, int font, bool ascii, string format, params object[] args )
		{
			Broadcast( hue, font, ascii, String.Format( format, args ) );
		}
		...
		private sealed class InstanceMapEntry : IEntityEntry {
			private BaseInstanceMap m_Map;
			private int m_TypeID;
			private string m_TypeName;
			private long m_Position;
			private int m_Length;

			public BaseInstanceMap Map {
				get {
					return m_Map;
				}
			}
			...
			public Serial Serial {
				get {
					return m_Map == null ? -1 : m_Map.MapIndex;
				}
			}
			...
			public InstanceMapEntry( BaseInstanceMap map, int typeID, string typeName, long pos, int length ) {
				m_Map = map;
				m_TypeID = typeID;
				m_TypeName = typeName;
				m_Position = pos;
		...
		private static void SaveIndex<T>( List<T> list, string path ) where T : IEntityEntry {
			...
			if ( !Directory.Exists( "Saves/Maps/" ) )
				Directory.CreateDirectory( "Saves/Maps/" );
		...
		public static void Save( bool message ) {
			...
			Console.WriteLine( "World: Saving started..." );
			...
			Console.WriteLine( "World: Saving done in {0:F2} seconds.", watch.Elapsed.TotalSeconds );
		...
		internal static List<Type> m_InstanceMapTypes = new List<Type>();
		...
		public static void AddMap( BaseInstanceMap map ) {
			if ( m_Saving ) {
				AppendSafetyLog( "add", map );
				_addMapQueue.Enqueue( map );
			} else {
				m_InstanceMaps[map.Serial] = map;
			}
		}
		...
		public static Map FindMap( Serial serial ) {
			BaseInstanceMap map;

			m_InstanceMaps.TryGetValue( serial, out map );
